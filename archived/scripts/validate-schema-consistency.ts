...
class ValidationConsistencyChecker {
  ...
  private async scanFile(filePath: string): Promise<void> {
    try {
      ...
      if (
        ...
        filePath.includes("lib/common/inputValidation")
      ) {
        return;
      }
  ...
  private checkDuplicatePatterns(content: string, filePath: string): void {
    ...
    lines.forEach((line, index) => {
      ...
      if (
        line.match(/z\.string\(\)\.min\(1.*?required.*?\)/i) &&
        !line.includes("createIdSchema")
      ) {
        this.issues.push({
          file: filePath,
          line: index + 1,
      ...
      if (
        line.match(/z\.string\(\)\.min\(1.*?name.*?required.*?\)\.max\(255/i)
      ) {
        this.issues.push({
          file: filePath,
          line: index + 1,
      ...
      if (
        line.match(/z\.string\(\)\.min\(1.*?title.*?required.*?\)\.max\(200/i)
      ) {
        this.issues.push({
          file: filePath,
          line: index + 1,
      ...
      if (line.match(/z\.string\(\)\.min\(1.*?comment.*?\)\.max\(2000/i)) {
        this.issues.push({
          ...
          pattern: line.trim(),
          suggestion:
            "Use `commentContentSchema` from ~/lib/validation/schemas",
          severity: "error",
        });
      }
      ...
      if (
        line.match(/z\.string\(\)\.email\(/i) &&
        !line.includes("emailSchema")
      ) {
        this.issues.push({
          file: filePath,
          line: index + 1,
    ...
    lines.forEach((line, index) => {
      ...
      if (maxMatches) {
        maxMatches.forEach((match) => {
          ...
          if (
            [200, 255, 320, 2000, 5000].includes(limit) &&
      ...
      if (line.includes("is required") || line.includes("required")) {
        ...
        if (
          ...
          this.issues.push({
            ...
            pattern: line.trim(),
            suggestion:
              "Consider using centralized schema with consistent error messages",
            severity: "info",
          });
        }
      }
  ...
  private reportIssues(): void {
    ...
    console.log(
      `❌ Found ${this.issues.length} validation consistency issues:\n`,
    );

    // Report errors first
    if (groupedIssues.duplicate) {
      console.log(
        "🚨 DUPLICATE VALIDATION PATTERNS (should use centralized schemas):",
      );
