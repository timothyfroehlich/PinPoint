...
describe("Mock System Usage Examples", () => {
  describe("Unit Tests - Using Individual Mocks", () => {
    it("demonstrates creating individual mock entities", () => {
      ...
      expect(mockUser.organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );
      expect(mockOrganization.id).toBe(SEED_TEST_IDS.ORGANIZATIONS.primary);
      expect(mockMachine.organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );
    ...
    it("demonstrates creating mocks with overrides", () => {
      ...
      expect(customUser.organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );
    });
  ...
  describe("Integration (DAL) - Using Auth and Database Mocks", () => {
    ...
    it("demonstrates mocking DAL functions with realistic data", async () => {
      ...
      expect(issues[0].organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );
      expect(
        issues.every(
          (issue) => issue.machine_id === SEED_TEST_IDS.MOCK_PATTERNS.MACHINE,
        ),
      ).toBe(true);
    });
    ...
    it("demonstrates cross-organization security testing", () => {
      ...
      expect(scenario.primaryOrg.issues[0].organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );

      // Competitor org should only see their data
      expect(scenario.competitorOrg.issues[0].organization_id).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.competitor,
      );

      // Organizations should be completely separate
      expect(scenario.primaryOrg.organization.id).not.toBe(
        scenario.competitorOrg.organization.id,
      );
    });
  });
  ...
  describe("Integration (Server Action) - Using FormData Mocks", () => {
    ...
    it("demonstrates status update FormData", () => {
      const formData =
        MockFormDataFactory.createValidStatusUpdateFormData("status-resolved");
  ...
  describe("Integration - Using Complete Scenarios", () => {
    it("demonstrates primary organization scenario", () => {
      ...
      expect(scenario.authContext.organizationId).toBe(
        scenario.organization.id,
      );
      expect(scenario.users[0].organization_id).toBe(scenario.organization.id);
      expect(scenario.machines[0].organization_id).toBe(
        scenario.organization.id,
      );
      expect(
        scenario.issues.every(
          (issue) => issue.organization_id === scenario.organization.id,
        ),
      ).toBe(true);
    });
    ...
    it("demonstrates edge case scenarios", () => {
      ...
      // Unassigned issues
      expect(
        edgeCases.unassignedIssues.issues.every(
  ...
  describe("Test Helper Patterns", () => {
    it("demonstrates consistent ID usage across tests", () => {
      ...
      const user2 = SeedBasedMockFactory.createMockUser({
        name: "Different Name",
      });
    ...
    it("demonstrates org scoping test patterns", () => {
      ...
      const competitorContext =
        MockAuthContextFactory.createCompetitorOrgContext();

      // Different organizations
      expect(primaryContext.organizationId).not.toBe(
        competitorContext.organizationId,
      );
      expect(primaryContext.organizationId).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.primary,
      );
      expect(competitorContext.organizationId).toBe(
        SEED_TEST_IDS.ORGANIZATIONS.competitor,
      );
