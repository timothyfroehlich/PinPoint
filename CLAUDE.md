...
**Two reviewers, either marker satisfies the gate.** Routine path is `./scripts/workflow/agy_review.py <PR>` (`--pro` for denser changes) — you run it yourself; it posts inline findings and writes the agy marker, recording its tier as the depth. Reserve Tim's `/code-review`, which you cannot launch, for auth, permissions, migrations, or architectural changes; finish your churn first, hand the branch over, then attest with `bash scripts/workflow/mark-claude-review.sh <PR> <depth> "<findings>"` (`<depth>` = the `/code-review` level he ran). Either marker pins a SHA, so a later push invalidates it. Full rules: `pinpoint-pr-workflow` Phase 3.4.
