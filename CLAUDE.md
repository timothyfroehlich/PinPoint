...
### Forbidden Commands
- `npm test 2>&1` / `npm test > file.txt` (breaks Vitest)
...
|-----------|------------------|--------|
| `npm test 2>&1` | `npm test` | Vitest treats redirection as test name filters |
| `find` command | `rg --files`, `fd` | Dangerous -exec flag |
