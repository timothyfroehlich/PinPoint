...
test.describe("Authentication Redirects", () => {
  test("unauthenticated user accessing apc org issues page shows error boundary or redirects", async ({ page }) => {
    ...
    if (currentUrl.includes('/auth/sign-in')) {
      ...
      const errorBoundary = page.locator("[data-testid='error-boundary']").or(
        page.locator("text=Authentication Error").or(
      ...
      if (await signInButton.isVisible().catch(() => false)) {
        ...
        await expect(page.locator("h1")).toContainText(/Welcome back|Sign In/i);
        console.log("✅ Sign In button successfully redirected to sign-in page");
      } else {
        // Look for any authentication-related error message
        const authErrorMessage = page.locator("text=/authentication|access.*required|member.*access|unauthorized/i");
  ...
  test("unauthenticated user accessing dashboard shows proper auth handling", async ({ page }) => {
    ...
    if (isOnSignIn) {
      ...
      console.log("❌ Not redirected from dashboard, checking for error boundary...");
      
      // Look for authentication error handling
      const authHandling = page.locator("text=/authentication|sign.*in|member.*access/i").or(
  ...
  test("unauthenticated access to localhost redirects to apc subdomain", async ({ page }) => {
