...
test.describe("prod: APC alias host", () => {
  test("root redirects to /auth/sign-in (no landing page)", async ({ page, baseURL }) => {
  ...
  test("sign-in page hides organization dropdown (org locked by host)", async ({ page, baseURL }) => {
    test.skip(!baseURL, "baseURL required");
    ...
    await expect(page.getByRole("heading", { name: "PinPoint" })).not.toBeVisible();
  });
});
