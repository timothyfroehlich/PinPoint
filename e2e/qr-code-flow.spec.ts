...
test.describe("QR Code to Issue Reporting Flow", () => {
  ...
  test("should navigate from machine QR code to issue detail page", async ({
    page,
  }) => {
    ...
    await expect(
      page
        .locator('text="Machine Details"')
        .or(page.locator('text="Game Information"')),
    )
      .toBeVisible()
      .catch(() => {
  ...
  test("should allow anonymous user to report an issue from machine page", async ({
    page,
  }) => {
    // CUJ 1.1: First-Time Discovery & Issue Reporting
    ...
    const reportIssueButton = page
      .locator("button", { hasText: "Report Issue" })
    ...
    await expect(
      page
        .locator('text="Report an Issue"')
        .or(page.locator('text="Create New Issue"')),
    ).toBeVisible();
  ...
  test("should allow photo attachment during issue reporting", async ({
    page,
  }) => {
  ...
  test("should show different UI for authenticated vs anonymous users", async ({
    page,
  }) => {
    ...
    const anonymousReportButton = await page
      .locator("button", { hasText: "Report Issue" })
      .isVisible()
      .catch(() => false);
    ...
    const authenticatedOptions = await page
      .locator("button", { hasText: "Report Issue" })
      .or(page.locator('text="Your Issues"'))
      .or(page.locator('text="Issue History"'))
      .isVisible()
      .catch(() => false);
  ...
  test("should handle duplicate issue reporting gracefully", async ({
    page,
  }) => {
    ...
    for (const indicator of locatorIndicators) {
  ...
  test("should navigate from QR code to existing issue detail", async ({
    page,
  }) => {
  ...
  test("should show machine context in issue reporting", async ({ page }) => {
    ...
    const machineContext = page
      .locator('text="test-machine-1"')
      .or(page.locator('text="Machine:"'))
      .or(page.locator('text="Location:"'))
      .or(page.locator('text="Game:"'));

    await expect(machineContext)
      .toBeVisible()
      .catch(() => {
  ...
  test("should provide clear navigation back to machine from issue", async ({
    page,
  }) => {
    ...
    if (await machineLink.isVisible().catch(() => false)) {
      ...
      expect(page.url()).toMatch(/\/machines\/|\/games\//);
    } else {
  ...
  test("should handle network errors gracefully during issue submission", async ({
    page,
  }) => {
    await page.goto("/machines/test-machine-1");
