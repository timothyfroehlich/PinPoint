...
test.describe('Issue Detail - Permission Matrix', () => {
  ...
  test.describe('Read Permissions', () => {
    test.fixme('allows users with issues:read to view issue details', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="edit-issue-button"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="status-dropdown"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="assign-user-button"]')).not.toBeVisible();
    });

    test.fixme('denies users without issues:read from viewing issues', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="permission-denied"]')).toContainText('You do not have permission to view this issue');
    });
  });

  test.describe('Edit Permissions', () => {
    test.fixme('shows edit button only with issues:edit permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="edit-issue-button"]')).toBeVisible();
    ...
    test.fixme('hides edit button without issues:edit permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read']);
      ...
      await expect(page.locator('[data-testid="disabled-edit-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="disabled-edit-button"]')).toHaveAttribute(
        'title',
        'You need edit permissions to modify this issue'
      );
    });

    test.fixme('allows status changes with issues:edit permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:edit']);
      ...
      await expect(page.locator('[data-testid="issue-status"]')).toContainText('In Progress');
      await expect(page.locator('[data-testid="status-change-activity"]')).toBeVisible();
    });
  });

  test.describe('Close Permissions', () => {
    test.fixme('shows close button only with issues:close permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="close-issue-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="mark-invalid-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="resolve-button"]')).toBeVisible();
    });

    test.fixme('hides close button without issues:close permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:edit']);
      ...
      await expect(page.locator('[data-testid="close-issue-button"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="resolve-button"]')).not.toBeVisible();
      
      // Should show disabled close button with tooltip
      await expect(page.locator('[data-testid="disabled-close-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="disabled-close-button"]')).toHaveAttribute(
        'title',
        'You need close permissions to close this issue'
      );
    });

    test.fixme('allows issue resolution with issues:close permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:close']);
      ...
      await expect(page.locator('[data-testid="resolution-note"]')).toContainText('Issue resolved by cleaning contacts');
    });
  });

  test.describe('Assign Permissions', () => {
    test.fixme('shows assign control only with issues:assign permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="assign-user-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="assign-to-self"]')).toBeVisible();
      await expect(page.locator('[data-testid="assign-to-other"]')).toBeVisible();
    });

    test.fixme('hides assign control without issues:assign permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:edit']);
      ...
      await expect(page.locator('[data-testid="disabled-assign-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="disabled-assign-button"]')).toHaveAttribute(
        'title',
        'You need assign permissions to assign this issue'
      );
    });

    test.fixme('allows self-assignment with issues:assign permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:assign']);
      ...
      await expect(page.locator('[data-testid="assigned-to"]')).toContainText('Test User');
      await expect(page.locator('[data-testid="assignment-activity"]')).toBeVisible();
    });
  });

  test.describe('Comment Permissions', () => {
    test.fixme('allows public comments with issues:comment permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await page.locator('[data-testid="comment-textarea"]').fill('Public comment from user');
      await page.locator('[data-testid="submit-comment-button"]').click();
      
      // Should see comment added
      await expect(page.locator('[data-testid="latest-comment"]')).toContainText('Public comment from user');
      await expect(page.locator('[data-testid="public-badge"]')).toBeVisible();
    });

    test.fixme('allows internal comments with issues:internal_comment permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await page.locator('[data-testid="comment-textarea"]').fill('Internal note for technicians');
      await page.locator('[data-testid="internal-comment-toggle"]').click();
      await page.locator('[data-testid="submit-comment-button"]').click();
      
      // Should see internal comment added
      await expect(page.locator('[data-testid="latest-comment"]')).toContainText('Internal note for technicians');
      await expect(page.locator('[data-testid="internal-badge"]')).toBeVisible();
    });

    test.fixme('hides internal comment toggle without issues:internal_comment permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:comment']);
      ...
      await page.locator('[data-testid="add-comment-button"]').click();
      await expect(page.locator('[data-testid="internal-comment-toggle"]')).not.toBeVisible();
      
      // Should only be able to add public comments
      await page.locator('[data-testid="comment-textarea"]').fill('Public comment only');
      await page.locator('[data-testid="submit-comment-button"]').click();
      
      await expect(page.locator('[data-testid="latest-comment"]')).toContainText('Public comment only');
      await expect(page.locator('[data-testid="public-badge"]')).toBeVisible();
    });

    test.fixme('hides comment form without issues:comment permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read']);
      ...
      await expect(page.locator('[data-testid="add-comment-button"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="comment-form"]')).not.toBeVisible();
      
      // Should show message about commenting
      await expect(page.locator('[data-testid="no-comment-permission"]')).toBeVisible();
      await expect(page.locator('[data-testid="no-comment-permission"]')).toContainText('You need comment permissions to add comments');
    });
  });

  test.describe('Delete Permissions', () => {
    test.fixme('shows delete controls only with issues:delete permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="delete-issue-button"]')).toBeVisible();
      await expect(page.locator('[data-testid="danger-actions"]')).toBeVisible();
    });

    test.fixme('hides delete controls without issues:delete permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:edit']);
      ...
      await expect(page.locator('[data-testid="delete-issue-button"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="danger-actions"]')).not.toBeVisible();
    });

    test.fixme('allows issue deletion with issues:delete permission', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read', 'issues:delete']);
  ...
  test.describe('Admin Permissions', () => {
    test.fixme('shows all controls with admin permissions', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="transfer-issue-button"]')).toBeVisible();
  ...
  test.describe('Role-based Access', () => {
    ...
    test.fixme('technician role shows extended permissions', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      ...
      await expect(page.locator('[data-testid="delete-issue-button"]')).not.toBeVisible();
      await expect(page.locator('[data-testid="transfer-issue-button"]')).not.toBeVisible();
    });
    ...
    test.fixme('owner role shows ownership permissions', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="priority-request-button"]')).toBeVisible();
  ...
  test.describe('Permission Tooltips and Feedback', () => {
    test.fixme('displays permission tooltips on disabled buttons', async ({ page }) => {
      // TODO: Implementation agent will complete this test
      // await loginAsUserWithPermissions(page, ['issues:read']);
    ...
    test.fixme('shows permission request options', async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="request-permissions"]')).toBeVisible();
      await expect(page.locator('[data-testid="request-edit-permission"]')).toBeVisible();
      
      // Should be able to request permissions
      await page.locator('[data-testid="request-edit-permission"]').click();
      await expect(page.locator('[data-testid="permission-request-sent"]')).toBeVisible();
    });
  });
});