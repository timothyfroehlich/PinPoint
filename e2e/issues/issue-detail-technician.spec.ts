...
import { loginAsTechnician, setupTestIssue } from "../helpers/auth";
...
test.describe("Issue Detail - Technician Workflow", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await setupTestIssue(page, {
      id: "test-issue-new",
      status: { id: "status-new", name: "New", color: "#ff0000" },
    });
  });

  test("manages complete issue lifecycle", async ({ page }) => {
    ...
    // Step 2: Verify issue is in "New" status
    await expect(page.locator('[data-testid="issue-status"]')).toContainText(
      "New",
    );

    // Step 3: Verify technician controls are visible
    ...
    // Step 5: Verify status change (this would require the actual implementation)
    // Note: Status change verification will work once the mutation is implemented
    ...
    // Step 7: Add work comment
    await page
      .locator('[data-testid="comment-textarea"]')
      .fill(
        "Started working on the issue. Found the problem with the flipper mechanism.",
      );
    await page.locator('[data-testid="submit-comment-button"]').click();
    await page
      .locator('[data-testid="photo-input"]')
      .setInputFiles("test-fixtures/repair-photo.jpg");

    // Submit comment
    await page.locator('[data-testid="submit-comment-button"]').click();

    // Step 10: Verify comment and photo were added
    await expect(page.locator('[data-testid="latest-comment"]')).toContainText(
    ...
    await page
      ...
      .fill("Replaced faulty flipper mechanism. Tested and working properly.");
    ...
    await expect(page.locator('[data-testid="resolution-note"]')).toContainText(
      "Replaced faulty flipper mechanism",
    );
    ...
    await expect(
      page.locator('[data-testid="activity-status-new"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="activity-status-acknowledged"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="activity-assignment"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="activity-status-in-progress"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="activity-comment"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="activity-status-resolved"]'),
    ).toBeVisible();
  });
  ...
  test.skip("handles daily triage workflow", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="acknowledge-button"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="close-invalid-button"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="merge-duplicate-button"]'),
    ).toBeVisible();
    ...
    await expect(page.locator('[data-testid="triaged-by"]')).toContainText(
      "Test Technician",
    );
  ...
  test.skip("merges duplicate issues", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="search-original-issue"]')
      .fill("Test Issue Original");
    ...
    await expect(
      page.locator('[data-testid="duplicate-details"]'),
    ).toContainText("Test Issue Duplicate");
    await expect(
      page.locator('[data-testid="original-details"]'),
    ).toContainText("Test Issue Original");
    ...
    await expect(page.locator('[data-testid="merge-activity"]')).toContainText(
      "Merged duplicate issue",
    );

    // Should see preserved duplicate details
    await expect(
      page.locator('[data-testid="duplicate-comments"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="duplicate-attachments"]'),
    ).toBeVisible();
  });
  ...
  test.skip("adds internal notes and comments", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="comment-textarea"]')
      .fill("Internal note: Need to order replacement parts");
    await page.locator('[data-testid="internal-comment-toggle"]').click();
    await page.locator('[data-testid="submit-comment-button"]').click();

    // Verify internal note is added
    await expect(page.locator('[data-testid="latest-comment"]')).toContainText(
      "Internal note: Need to order replacement parts",
    );
    ...
    await page
      .locator('[data-testid="comment-textarea"]')
      .fill("Updated customer on repair status");
    // Internal toggle should be off by default
    await page.locator('[data-testid="submit-comment-button"]').click();

    // Verify public comment is added
    await expect(page.locator('[data-testid="latest-comment"]')).toContainText(
      "Updated customer on repair status",
    );
  ...
  test.skip("handles bulk status changes", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="bulk-success-message"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="bulk-success-message"]'),
    ).toContainText("5 issues updated");
  ...
  test.skip("manages issue priority changes", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="priority-reason-textarea"]')
      .fill("Customer reported loss of revenue");
    ...
    await expect(
      page.locator('[data-testid="priority-indicator"]'),
    ).toHaveClass(/priority-high/);

    // Verify priority change activity
    await expect(
      page.locator('[data-testid="priority-change-activity"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="priority-change-activity"]'),
    ).toContainText("Customer reported loss of revenue");
  });
  ...
  test.skip("handles issue escalation workflow", async ({ page }) => {
    ...
    await page
      .locator('[data-testid="escalation-reason"]')
      .fill("Requires advanced electrical troubleshooting");
    await page.locator('[data-testid="confirm-escalation"]').click();

    // Verify escalation
    await expect(
      page.locator('[data-testid="escalation-badge"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="escalated-to"]')).toContainText(
      "Senior Technician",
    );
    await expect(
      page.locator('[data-testid="escalation-activity"]'),
    ).toBeVisible();
  ...
  test.skip("validates required fields before status changes", async ({
    page,
  }) => {
    // TODO: Implementation agent will complete this test
    ...
    await expect(
      page.locator('[data-testid="validation-error"]'),
    ).toContainText("Resolution notes are required");
  ...
  test.skip("shows technician-specific UI elements", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="technician-toolbar"]'),
    ).toBeVisible();
    await expect(page.locator('[data-testid="quick-actions"]')).toBeVisible();
    await expect(
      page.locator('[data-testid="internal-notes-section"]'),
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="work-log-section"]'),
    ).toBeVisible();
  ...
  test.skip("handles offline scenarios gracefully", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="queued-changes"]')).toContainText(
      "1 change queued",
    );
