...
test.describe('Issue Detail - Public User', () => {
  test.beforeEach(async ({ page }) => {
    // TODO: Implementation agent should set up proper test data
    // For now, these tests will fail until implementation is complete
  });

  test.fixme('navigates from QR code to issue detail page', async ({ page }) => {
  ...
  test.fixme('displays issue information correctly for public view', async ({ page }) => {
    // TODO: Implementation agent will complete this test
    await page.goto('/issues/test-issue-1');
    
    // Verify basic issue information is displayed
    await expect(page.locator('[data-testid="issue-title"]')).toContainText('Test Issue Title');
    await expect(page.locator('[data-testid="issue-description"]')).toContainText('Test issue description');
    await expect(page.locator('[data-testid="issue-status"]')).toContainText('Open');
    await expect(page.locator('[data-testid="issue-priority"]')).toContainText('Medium');
    
    // Verify machine information is displayed
    await expect(page.locator('[data-testid="machine-name"]')).toContainText('Test Game');
    await expect(page.locator('[data-testid="machine-location"]')).toContainText('Test Location');
    await expect(page.locator('[data-testid="machine-serial"]')).toContainText('TEST123');
    
    // Verify timestamps are displayed
    await expect(page.locator('[data-testid="issue-created-date"]')).toBeVisible();
    await expect(page.locator('[data-testid="issue-last-updated"]')).toBeVisible();
  });

  test.fixme('shows only public comments for unauthenticated users', async ({ page }) => {
    // TODO: Implementation agent will complete this test
    await page.goto('/issues/test-issue-with-mixed-comments');
    
    // Should see public comments
    await expect(page.locator('[data-testid="public-comment-1"]')).toBeVisible();
    await expect(page.locator('[data-testid="public-comment-1"]')).toContainText('This is a public comment');
    
    // Should NOT see internal comments
    await expect(page.locator('[data-testid="internal-comment-1"]')).not.toBeVisible();
    await expect(page.locator('.internal-comment')).not.toBeVisible();
    
    // Should NOT see internal comment indicators
    await expect(page.locator('[data-testid="internal-badge"]')).not.toBeVisible();
  });
  ...
  test.fixme('displays image attachments correctly', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="attachment-thumbnail"]')).toBeVisible();
  ...
  test.fixme('handles non-existent issues gracefully', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="issue-not-found"]')).toContainText('Issue not found');
    
    // Should provide helpful actions
    await expect(page.locator('[data-testid="back-to-dashboard"]')).toBeVisible();
    await expect(page.locator('[data-testid="report-new-issue"]')).toBeVisible();
  });
  ...
  test.fixme('shows login prompt for protected actions', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="login-modal"]')).toContainText('Please sign in to comment');
    
    // Should have login options
    await expect(page.locator('[data-testid="email-login-button"]')).toBeVisible();
  ...
  test.fixme('supports keyboard navigation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="add-comment-button"]')).toBeFocused();
    
    await page.keyboard.press('Tab');
    await expect(page.locator('[data-testid="attachment-thumbnail"]').first()).toBeFocused();
  ...
  test.fixme('displays correctly on mobile devices', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="mobile-actions-bar"]')).toBeVisible();
    await expect(page.locator('[data-testid="mobile-share-button"]')).toBeVisible();
  });
  ...
  test.fixme('shows proper loading states', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="comments-skeleton"]')).toBeVisible();
    await expect(page.locator('[data-testid="timeline-skeleton"]')).toBeVisible();
    
    // Wait for content to load
    await expect(page.locator('[data-testid="issue-title"]')).toBeVisible({ timeout: 5000 });
    
    // Skeletons should be hidden
    await expect(page.locator('[data-testid="issue-skeleton"]')).not.toBeVisible();
  });
  ...
  test.fixme('handles network errors gracefully', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="network-error"]')).toContainText('Failed to load issue');
  ...
  test.fixme('provides proper meta tags for SEO and sharing', async ({ page }) => {
    // TODO: Implementation agent will complete this test
    await page.goto('/issues/test-issue-1');
    ...
    await expect(metaDescription).toHaveAttribute('content', /Test issue description/);
    ...
    await expect(ogDescription).toHaveAttribute('content', /Test issue description/);
