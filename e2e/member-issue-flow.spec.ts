...
test.describe("Member Issue Reporting Flow", () => {
  ...
  test("should report an issue while logged in with identity attached", async ({
    page,
  }) => {
    ...
    const createIssueButton = page
      .locator("button", { hasText: "Create Issue" })
    ...
    if (await createIssueButton.isVisible().catch(() => false)) {
      ...
      expect(page.url()).toMatch(
        /\/issues\/create|\/issues\/new|\/create|\/report/,
      );
      ...
      if (await descriptionField.isVisible().catch(() => false)) {
        await descriptionField.fill(
          "This issue was created by an authenticated user to test the member reporting flow.",
        );
      }

      // Look for machine/location selection
      const machineSelector = page
        .locator("select")
      ...
      if (await submitButton.isVisible().catch(() => false)) {
        ...
        expect(
          hasSuccess || hasError || page.url() !== page.url(),
        ).toBeTruthy();
      }
    }
  ...
  test("should navigate to profile page and update user information", async ({
    page,
  }) => {
    ...
    if (await profileLink.isVisible().catch(() => false)) {
      ...
      expect(page.url()).toMatch(/\/profile|\/account|\/settings/);
      ...
      if (await avatarUpload.isVisible().catch(() => false)) {
        ...
        void expect(avatarUpload).toBeVisible();
      }
    } else {
  ...
  test("should track and monitor reported issues", async ({ page }) => {
    ...
    const myIssuesSection = page
      .locator('text="My Issues"')
    ...
    if (await myIssuesSection.isVisible().catch(() => false)) {
      ...
      expect(page.url()).toMatch(/\/issues|\/dashboard/);

      // Look for issue items
      const issueItems = page
  ...
  test("should show enhanced issue creation form for authenticated users", async ({
    page,
  }) => {
    ...
    const authenticatedCreateButton = page
      .locator("button", { hasText: "Create Issue" })
      .or(page.locator("button", { hasText: "Report Issue" }))
      .or(page.locator('a[href*="/issues/create"]'));
  ...
  test("should allow viewing and updating issue status", async ({ page }) => {
    ...
    if (await issueLinks.isVisible().catch(() => false)) {
      ...
      expect(page.url()).toMatch(/\/issues\/\w+/);

      // Should see issue details
      const issueDetails = page
        .locator('text="Status"')
      ...
      if (await interactionOptions.isVisible().catch(() => false)) {
        ...
        void expect(interactionOptions).toBeVisible();
      }
    } else {
  ...
  test("should handle member logout and session management", async ({
    page,
  }) => {
    await loginAsRegularUser(page);
    await page.goto("/dashboard");
  ...
  test("should display appropriate error messages for member actions", async ({
    page,
  }) => {
    await loginAsRegularUser(page);
    await page.goto("/dashboard");
    ...
    for (const route of adminRoutes) {
      ...
      const hasError = await page
        ...
        .or(page.locator('text="Permission"'))
        .isVisible()
        .catch(() => false);
