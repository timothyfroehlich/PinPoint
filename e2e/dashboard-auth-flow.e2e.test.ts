...
test.describe("Dashboard Authentication Flow (E2E Tests - Archetype 7)", () => {
  test.describe("Authentication and authorization", () => {
    test("dev login flow loads dashboard without authentication errors", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator("text=Welcome back, Tim Froehlich"),
      ).toBeVisible();
      ...
      const statsSection = page.locator(
        '[data-testid="dashboard-stats"], .grid:has-text("Total Machines")',
      );
      await expect(statsSection.first()).toBeVisible();
    });

    test("dashboard preserves authentication across page refreshes", async ({
      page,
    }) => {
      // Perform login
      await page.goto("/auth/sign-in");
    ...
    test("unauthenticated users cannot access dashboard directly", async ({
      page,
    }) => {
  ...
  test.describe("Server Component rendering", () => {
    ...
    test("Server Components load stats without auth errors", async ({
      page,
    }) => {
  ...
  test.describe("Navigation and workflow continuity", () => {
    ...
    test("navigation works after successful authentication", async ({
      page,
    }) => {
      await page.goto("/dashboard");
  ...
  test.describe("Progressive enhancement", () => {
    test("works without JavaScript (Server Components)", async ({
      browser,
    }) => {
      ...
      await expect(
        noJsPage.locator("text=Welcome back, Tim Froehlich"),
      ).toBeVisible();
  ...
  test.describe("Performance and caching", () => {
    ...
    test("handles multiple concurrent requests without auth errors", async ({
      page,
    }) => {
      const consoleErrors: string[] = [];
      page.on("console", (msg) => {
