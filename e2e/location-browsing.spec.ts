...
test.describe("Location Browsing Flow", () => {
  ...
  test("should navigate from homepage to location selection", async ({ page }) => {
  ...
  test("should browse available machines at a location", async ({ page }) => {
    ...
    for (const url of locationUrls) {
      ...
      const machineElements = page.locator('text="Game"')
      ...
      if (await machineElements.isVisible().catch(() => false)) {
        ...
        const machineNames = page.locator('text*="Medieval Madness"')
  ...
  test("should filter and search for specific machines", async ({ page }) => {
    ...
    const filterButtons = page.locator('button', { hasText: "Filter" })
    ...
    if (await filterButtons.isVisible().catch(() => false)) {
      ...
      const filterOptions = page.locator('text="Name"')
  ...
  test("should display machine details when clicked", async ({ page }) => {
    ...
    const machineItems = page.locator('a[href*="/machines"]')
    ...
    if (await machineItems.isVisible().catch(() => false)) {
      ...
      const machineDetails = page.locator('text="Details"')
        .or(page.locator('text="Information"'))
        .or(page.locator('text="Status"'))
        .or(page.locator('text="Location"'))
        .or(page.locator('text="Issues"'));
      
      await expect(machineDetails).toBeVisible().catch(() => {
  ...
  test("should show different content for authenticated vs anonymous users", async ({ page }) => {
    // Test anonymous browsing first
    await page.goto("/dashboard");
    
    const anonymousContent = await page.locator('text="Sign in"')
    ...
    const authenticatedContent = await page.locator('button[aria-label="account of current user"]')
      .or(page.locator('text="Profile"'))
      .or(page.locator('text="Settings"')).isVisible().catch(() => false);
  ...
  test("should handle mobile responsive design", async ({ page }) => {
    ...
    const content = page.locator('text="Dashboard"')
  ...
  test("should handle empty or loading states gracefully", async ({ page }) => {
    ...
    const hasContent = await page.locator('text="Dashboard"')
      .or(page.locator('text="PinPoint"')).isVisible().catch(() => false);
    
    expect(hasLoading || hasContent).toBeTruthy();
    
    // Should eventually show content (wait up to 5 seconds)
    await expect(page.locator('text="Dashboard"')
      .or(page.locator('text="PinPoint"'))).toBeVisible({ timeout: 5000 });
  });
  ...
  test("should display machine status and issue indicators", async ({ page }) => {
    await page.goto("/dashboard");
    
    // Look for status indicators
    const statusIndicators = page.locator('text="Working"')
      .or(page.locator('text="Issues"'))
