-- Sync handle_new_user trigger defaults with schema changes from 0013_mute_korg.
-- The trigger was hardcoding notification preferences to 'true', but schema defaults
-- were changed to 'false' in migration 0013. Also adds the new suppress_own_actions column.
...
COMMENT ON FUNCTION public.handle_new_user() IS
  'Automatically creates a user_profile and notification_preferences when a new user signs up via Supabase Auth. Works for both email/password and OAuth (Google, GitHub). Notification defaults are synced with Drizzle schema (see migration 0014). Also transfers guest issues (by reporter_email) and handles legacy invited_users cleanup.';
