...
export default {
  meta: {
    ...
    docs: {
      description:
        "Detect async server functions that should use cache() wrapper",
    ...
    messages: {
      missingCacheWrapper:
        'Async server function "{{functionName}}" should be wrapped in cache() per CORE-PERF-001. Use: export const {{functionName}} = cache(async (...) => { ... });',
      missingCacheImport:
        'cache() wrapper used but React cache not imported. Add: import { cache } from "react";',
    },
  },
    ...
    function checkAsyncExport(node) {
      ...
      if (node.type === "ExportNamedDeclaration") {
        if (node.declaration) {
          ...
          // export async function fn() {}
            ...
            isAsyncFunction(node.declaration)
          ) {
      ...
      // Direct variable declarations that are exported
        ...
        if (declarator && declarator.init && isAsyncFunction(declarator.init)) {
          ...
          if (
            text.includes(`export { ${varName}`) ||
            text.includes(`export {${varName}`)
          ) {
            if (
              !isCacheWrapped(declarator.init) &&
              shouldFunctionBeCached(varName)
            ) {
              context.report({
                node: declarator.init,
                messageId: "missingCacheWrapper",
    ...
    return {
      ...
      ImportDeclaration(node) {
        if (node.source.value === "react") {
          const cacheImport = node.specifiers.find(
            (spec) =>
