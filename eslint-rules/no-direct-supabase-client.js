...
const FORBIDDEN_IMPORTS = [
  {
    ...
    message: 'Use createClient from ~/lib/supabase/server for SSR-compatible client per CORE-SSR-001'
  },
  {
    module: '@supabase/auth-helpers-nextjs',
    importName: '*',
    message: 'auth-helpers-nextjs is deprecated. Use ~/lib/supabase/server createClient() wrapper instead'
  }
];
...
export default {
  meta: {
    type: 'problem',
    docs: {
      description: 'Prevent direct Supabase client usage, enforce SSR wrapper usage',
            ...
            FORBIDDEN_IMPORTS.forEach(forbiddenImport => {
              if (moduleName === forbiddenImport.module) {
                context.report({
                  node,
                  messageId: 'directSupabaseImport',
                  data: { message: `CommonJS require: ${forbiddenImport.message}` }
                });
              }
            });
          }
