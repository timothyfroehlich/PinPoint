...
describe("Permission Constants", () => {
  ...
  describe("PERMISSION_DEPENDENCIES", () => {
    it("should define logical permission hierarchies", () => {
      ...
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.ISSUE_EDIT]).toContain(PERMISSIONS.ISSUE_VIEW);
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.MACHINE_EDIT]).toContain(PERMISSIONS.MACHINE_VIEW);
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.LOCATION_EDIT]).toContain(PERMISSIONS.LOCATION_VIEW);

      // Delete permissions should depend on view permissions
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.ISSUE_DELETE]).toContain(PERMISSIONS.ISSUE_VIEW);
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.MACHINE_DELETE]).toContain(PERMISSIONS.MACHINE_VIEW);
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.LOCATION_DELETE]).toContain(PERMISSIONS.LOCATION_VIEW);

      // Bulk operations should depend on individual operations
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.ISSUE_BULK_MANAGE]).toContain(PERMISSIONS.ISSUE_VIEW);
      expect(PERMISSION_DEPENDENCIES[PERMISSIONS.ISSUE_BULK_MANAGE]).toContain(PERMISSIONS.ISSUE_EDIT);
    });

    it("should only reference valid permissions", () => {
      const allPermissionValues = Object.values(PERMISSIONS);

      Object.entries(PERMISSION_DEPENDENCIES).forEach(([permission, dependencies]) => {
    ...
    it("should have consistent dependency structure", () => {
      ...
      const editPermissions = Object.values(PERMISSIONS).filter(p => p.includes(":edit"));
      ...
      editPermissions.forEach((editPermission) => {
        ...
        if (resource) {
          ...
          if (Object.values(PERMISSIONS).includes(viewPermission)) {
            expect(PERMISSION_DEPENDENCIES[editPermission]).toContain(viewPermission);
          }
        }
      });
    });
  ...
  describe("Derived constants", () => {
    ...
    describe("UNAUTHENTICATED_PERMISSIONS", () => {
      it("should include only safe permissions for public access", () => {
        ...
        expect(UNAUTHENTICATED_PERMISSIONS).toContain(PERMISSIONS.LOCATION_VIEW);
        
        // Should include safe create permissions
        expect(UNAUTHENTICATED_PERMISSIONS).toContain(PERMISSIONS.ISSUE_CREATE);
        expect(UNAUTHENTICATED_PERMISSIONS).toContain(PERMISSIONS.ATTACHMENT_CREATE);
        
        // Should NOT include edit/delete/admin permissions
        expect(UNAUTHENTICATED_PERMISSIONS).not.toContain(PERMISSIONS.ISSUE_EDIT);
        expect(UNAUTHENTICATED_PERMISSIONS).not.toContain(PERMISSIONS.ISSUE_DELETE);
        expect(UNAUTHENTICATED_PERMISSIONS).not.toContain(PERMISSIONS.ROLE_MANAGE);
        expect(UNAUTHENTICATED_PERMISSIONS).not.toContain(PERMISSIONS.USER_MANAGE);
      });
