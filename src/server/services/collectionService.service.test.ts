 ...
 * 2. Replace `CollectionService` with actual service class name
...
// TRPCError import removed - not used in current tests
...
describe("CollectionService (Service Tests)", () => {
  ...
  beforeEach(() => {
    ...
    service = new CollectionService(context.mockDb); // Some services: (context.mockDb, context.organizationId)
  });
  ...
  describe("Constructor and Organization Scoping", () => {
    it("should initialize with correct dependency injection", () => {
      ...
      const serviceInstance = new CollectionService(mockDbClient); // CUSTOMIZE: Update constructor
      expect(serviceInstance).toBeInstanceOf(CollectionService);
    });
    ...
    it("should create separate service instances with different database clients", () => {
      ...
      const primaryService = new CollectionService(primaryDbClient); // CUSTOMIZE: Update constructor
      const secondaryService = new CollectionService(secondaryDbClient); // CUSTOMIZE: Update constructor

      // Assert: Services are independent instances
      expect(primaryService).not.toBe(secondaryService);
      expect(primaryService).toBeInstanceOf(CollectionService);
      expect(secondaryService).toBeInstanceOf(CollectionService);
    });
  });
  ...
  describe("getLocationCollections", () => {
    ...
    it("should handle successful business logic execution", async () => {
      ...
      context.mockDb.query.collections.findFirst.mockResolvedValue(
        mockResponse,
      );

      // Act
      const result = await service.getLocationCollections("location-1");

      // Assert: Verify business logic result
      expect(result).toEqual(mockResponse);
      expect(context.mockDb.query.collections.findFirst).toHaveBeenCalledTimes(
        1,
      );
    });
    ...
    it("should handle not found scenarios", async () => {
      ...
      context.mockDb.query.collections.findFirst.mockResolvedValue(null);
    ...
    it("should handle database errors gracefully", async () => {
      ...
      context.mockDb.query.collections.findFirst.mockRejectedValue(dbError);

      // Act & Assert: Service should propagate database errors
      await expect(
        service.getLocationCollections("test-location"),
    ...
    it("should validate business rules", async () => {
      ...
      await expect(service.getLocationCollections("")).rejects.toThrow(
        "Invalid input",
      );
    });
  });
  ...
  describe("createManualCollection", () => {
    it("should create entity with valid data", async () => {
      ...
      const result = await service.createManualCollection(createData);
    ...
    it("should throw error when creation fails", async () => {
      ...
      await expect(service.createManualCollection(createData)).rejects.toThrow(
  ...
  describe("toggleCollectionType", () => {
    it("should update entity successfully", async () => {
      ...
      // Act
      await service.toggleCollectionType("type-1", true);
  ...
  describe("getCollectionMachines", () => {
    ...
    it("should execute raw SQL operations", async () => {
      ...
      // Act
      const _result = await service.getCollectionMachines(
        "collection-1",
        "location-1",
      );
  ...
  describe("Multi-tenant Organization Isolation", () => {
    ...
    it("should enforce organization scoping in business logic", async () => {
      ...
      const primaryService = new CollectionService(context.mockDb); // CUSTOMIZE constructor
      const competitorService = new CollectionService(context.mockDb); // CUSTOMIZE constructor

      // Mock database responses
      context.mockDb.query.collections.findMany.mockResolvedValue([
      ...
      expect(context.mockDb.query.collections.findMany).toHaveBeenCalledTimes(
        2,
      );
    });
  ...
  describe("Business Logic Validation", () => {
    it("should apply business rules correctly", async () => {
      ...
      context.mockDb.query.collectionTypes.findMany.mockResolvedValue([
  ...
  describe("Type Safety and Mock Validation", () => {
    it("should maintain type safety with service responses", () => {
      ...
      const typedService = new CollectionService(context.mockDb); // CUSTOMIZE constructor

      // Assert: TypeScript ensures type safety
      expect(typedService).toBeInstanceOf(CollectionService);
    });

    it("should demonstrate service dependency injection pattern", () => {
      // Act: Service accepts injected dependencies
      const mockDbClient = serviceTestUtils.mockDatabase();
      const serviceInstance = new CollectionService(mockDbClient); // CUSTOMIZE constructor
