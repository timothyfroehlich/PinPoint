...
describe("PermissionService - expandPermissionsWithDependencies", () => {
  ...
  describe("with simple dependencies", () => {
    it("should include dependencies for single permission", () => {
      const result = permissionService.expandPermissionsWithDependencies(["issue:edit"]);
      
      expect(result).toEqual(expect.arrayContaining(["issue:edit", "issue:view"]));
  ...
  describe("with complex dependencies", () => {
    it("should handle multi-level dependencies", () => {
      const result = permissionService.expandPermissionsWithDependencies(["issue:bulk_manage"]);
    ...
    it("should handle permissions that are both base and dependency", () => {
      ...
      expect(result).toEqual(expect.arrayContaining(["issue:view", "issue:edit"]));
      expect(result).toHaveLength(2);
    });
  });
  ...
  describe("edge cases", () => {
    it("should handle permissions with no matching dependencies", () => {
      const result = permissionService.expandPermissionsWithDependencies(["unknown:permission"]);
  ...
  describe("return value characteristics", () => {
    ...
    it("should always include original permissions in result", () => {
      const originalPermissions = ["issue:edit", "machine:create", "unknown:permission"];
      const result = permissionService.expandPermissionsWithDependencies(originalPermissions);
    ...
    it("should return array type", () => {
      const result = permissionService.expandPermissionsWithDependencies(["issue:view"]);
  ...
  describe("specific dependency scenarios", () => {
    it("should expand attachment permissions correctly", () => {
      const result = permissionService.expandPermissionsWithDependencies(["attachment:delete"]);
    ...
    it("should handle permissions without dependencies correctly", () => {
      ...
      expect(result).toEqual(expect.arrayContaining(["issue:view", "machine:view"]));
      expect(result).toHaveLength(2);
    });
    ...
    it("should handle complex permission sets", () => {
      ...
      const result = permissionService.expandPermissionsWithDependencies(complexSet);
