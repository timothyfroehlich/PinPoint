...
export class PermissionService {
  ...
  async requirePermission(
    ...
    const hasAccess = await this.hasPermission(session, permission, organizationId);
  ...
  private async checkUnauthenticatedPermission(
    ...
    const expandedPermissions = this.expandPermissionsWithDependencies(rolePermissions);
   ...
   */
  private async getUnauthenticatedPermissions(organizationId?: string): Promise<string[]> {
    if (!organizationId) {
   ...
   */
  private roleHasPermission(role: any, permission: string): boolean {
