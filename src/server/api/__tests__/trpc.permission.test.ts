...
const createMockTRPCContext = (
  ...
  mockContext.db.membership.findFirst.mockResolvedValue(mockMembership);
...
const createTestRouter = (): ReturnType<typeof createTRPCRouter> => {
  return createTRPCRouter({
    ...
    testIssueAssign: issueAssignProcedure
      ...
      .mutation(({ input }) => {
        return {
          message: `Issue assigned: ${input.issueId} to ${input.userId}`,
        };
      }),
...
// Helper type for test router caller with proper typing
interface TestRouterCaller {
  testRequirePermission: () => Promise<{ message: string }>;
  testIssueCreate: (input: { title: string }) => Promise<{ message: string }>;
  testIssueEdit: (input: {
    id: string;
    title: string;
  }) => Promise<{ message: string }>;
  testIssueDelete: (input: { id: string }) => Promise<{ message: string }>;
  testIssueAssign: (input: {
    issueId: string;
    userId: string;
  }) => Promise<{ message: string }>;
  testAttachmentCreate: (input: {
    filename: string;
  }) => Promise<{ message: string }>;
  testAttachmentDelete: (input: { id: string }) => Promise<{ message: string }>;
  testMachineEdit: (input: {
    id: string;
    serialNumber: string;
  }) => Promise<{ message: string }>;
  testMachineDelete: (input: { id: string }) => Promise<{ message: string }>;
  testLocationEdit: (input: {
    id: string;
    name: string;
  }) => Promise<{ message: string }>;
  testLocationDelete: (input: { id: string }) => Promise<{ message: string }>;
  testOrganizationManage: (input: {
    name: string;
  }) => Promise<{ message: string }>;
  testRoleManage: (input: { roleName: string }) => Promise<{ message: string }>;
  testUserManage: (input: { userId: string }) => Promise<{ message: string }>;
  testEmptyPermission?: () => Promise<{ message: string }>;
}
...
describe("tRPC Permission Middleware", () => {
  ...
  describe("requirePermission factory", () => {
    it("should allow access when user has required permission", async () => {
      ...
      const caller = testRouter.createCaller(ctx) as TestRouterCaller;
  ...
  describe("Issue Permission Procedures", () => {
    ...
    it("should deny issue creation without issue:create permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testIssueCreate({ title: "Test Issue" }),
    ...
    it("should allow issue editing with issue:edit permission", async () => {
      ...
      // Act
      const result = await caller.testIssueEdit({
        id: "issue-1",
        title: "Updated Issue",
      });
    ...
    it("should deny issue editing without issue:edit permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testIssueEdit({ id: "issue-1", title: "Updated Issue" }),
    ...
    it("should deny issue deletion without issue:delete permission", async () => {
      ...
      // Act & Assert
      await expect(caller.testIssueDelete({ id: "issue-1" })).rejects.toThrow(
        "Permission required: issue:delete",
      );
    });
    ...
    it("should allow issue assignment with issue:assign permission", async () => {
      ...
      // Act
      const result = await caller.testIssueAssign({
        issueId: "issue-1",
        userId: "user-2",
      });
    ...
    it("should deny issue assignment without issue:assign permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testIssueAssign({ issueId: "issue-1", userId: "user-2" }),
      ).rejects.toThrow("Permission required: issue:assign");
    });
  });
  ...
  describe("Attachment Permission Procedures", () => {
    it("should allow attachment creation with attachment:create permission", async () => {
      ...
      // Act
      const result = await caller.testAttachmentCreate({
        filename: "test.jpg",
      });
    ...
    it("should deny attachment creation without attachment:create permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testAttachmentCreate({ filename: "test.jpg" }),
      ).rejects.toThrow("Permission required: attachment:create");
    });
    ...
    it("should allow attachment deletion with attachment:delete permission", async () => {
      ...
      // Act
      const result = await caller.testAttachmentDelete({
        id: "attachment-1",
      });
    ...
    it("should deny attachment deletion without attachment:delete permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testAttachmentDelete({ id: "attachment-1" }),
      ).rejects.toThrow("Permission required: attachment:delete");
    });
  });
  ...
  describe("Machine Permission Procedures", () => {
    it("should allow machine editing with machine:edit permission", async () => {
      ...
      // Act
      const result = await caller.testMachineEdit({
        id: "machine-1",
        serialNumber: "ABC123",
      });
    ...
    it("should deny machine editing without machine:edit permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testMachineEdit({ id: "machine-1", serialNumber: "ABC123" }),
    ...
    it("should deny machine deletion without machine:delete permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testMachineDelete({ id: "machine-1" }),
      ).rejects.toThrow("Permission required: machine:delete");
    });
  });
  ...
  describe("Location Permission Procedures", () => {
    it("should allow location editing with location:edit permission", async () => {
      ...
      // Act
      const result = await caller.testLocationEdit({
        id: "location-1",
    ...
    it("should deny location deletion without location:delete permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testLocationDelete({ id: "location-1" }),
      ).rejects.toThrow("Permission required: location:delete");
    });
  });
  ...
  describe("Administrative Permission Procedures", () => {
    it("should allow organization management with organization:manage permission", async () => {
      ...
      // Act
      const result = await caller.testOrganizationManage({
        name: "Updated Organization",
      });

      // Assert
      expect(result).toEqual({
        message: "Organization managed: Updated Organization",
      });
    });
    ...
    it("should deny organization management without organization:manage permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testOrganizationManage({ name: "Updated Organization" }),
    ...
    it("should deny role management without role:manage permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testRoleManage({ roleName: "New Role" }),
    ...
    it("should deny user management without user:manage permission", async () => {
      ...
      // Act & Assert
      await expect(
        caller.testUserManage({ userId: "user-123" }),
  ...
  describe("Multiple Permission Scenarios", () => {
    it("should allow access with multiple permissions including the required one", async () => {
      ...
      // Act
      const createResult = await caller.testIssueCreate({
        title: "Test Issue",
      });
      const editResult = await caller.testIssueEdit({
        id: "issue-1",
        title: "Updated Issue",
      });
      const deleteResult = await caller.testIssueDelete({ id: "issue-1" });
      const orgResult = await caller.testOrganizationManage({
        name: "Org Name",
      });
  ...
  describe("Permission Middleware Edge Cases", () => {
    ...
    it("should handle empty string permission", async () => {
      ...
      const ctx = createMockTRPCContext([""]);
      const caller = emptyPermissionRouter.createCaller(
        ctx,
      ) as TestRouterCaller;
