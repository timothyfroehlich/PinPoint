...
import { generateCacheKey, isValidOPDBId, formatMachineName } from './utils';
...
class SimpleOPDBClient {
  ...
  async searchMachines(query: string): Promise<OPDBSearchResult[]> {
    ...
    const result = await this.request<OPDBSearchResponse>(`/search?q=${encodeURIComponent(query)}`);
  ...
  async getMachineById(opdbId: string): Promise<OPDBMachineDetails | null> {
    ...
    const result = await this.request<OPDBMachineDetails>(`/machines/${opdbId}`);
...
async function testSimpleOPDBAPI() {
  ...
  try {
    ...
    console.log(`   ✅ Search for "medieval" returned ${searchResult.length} results`);

    if (searchResult.length > 0) {
      console.log(`   Top result: "${searchResult[0]?.text}" (ID: ${searchResult[0]?.id})`);
    }
    console.log();
    ...
    if (machine) {
      ...
      console.log(`      Description: ${machine.description?.substring(0, 100) || 'N/A'}...`);
