...
export interface McpCatalogEditionResult {
  ...
  /**
   * WHOSE editions these are — the family's display name, and the ONLY reliable
   * check that the lookup returned what the caller meant.
   *
   * The request is a bare integer, and PBM's machine ids and machine-group ids
   * are separate id spaces that overlap numerically: group ids are small and
   * dense (8–137 in our fixture) while catalog machine ids run from 1 to ~10k,
   * so passing an edition's `pinballmapMachineId` here can match a real,
   * unrelated family. Nothing about the integer distinguishes the two — only the
   * name of what came back does.
   *
   * That overlap is also why there is deliberately NO "this id is also a machine
   * id" flag: it would be true for nearly every CORRECT group id, so it carries
   * no signal and would train a caller to discard good results. Don't add one.
   *
   * `null` when the mirror holds the group but never captured its name (PBM
   * serves group names from a separate endpoint) — the editions' own titles are
   * then the check.
   */
...
export async function runSearchPinballmapCatalog(
  ...
  if (machineGroupId !== undefined) {
    ...
    // Say whose editions these are. A bare integer request cannot be checked by
    // the caller unless the answer names itself — and the name is the whole
    // check, since the two id spaces overlap (see `familyName`).
    const familyName = await getGroupName(machineGroupId);
