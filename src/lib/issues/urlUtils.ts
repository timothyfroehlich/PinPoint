 ...
 */
export function urlParamsToFilters(params: URLSearchParams): Partial<IssueFilters> {
...
export function hasValidFilterParams(params: URLSearchParams): boolean {
  const validKeys = [
    'locationId', 'machineId', 'statusIds', 'search', 
    'assigneeId', 'reporterId', 'ownerId', 'sortBy', 'sortOrder'
  ];
 ...
 */
function isValidSortBy(value: string | null): value is "created" | "updated" | "status" | "severity" | "game" {
 ...
 */
export function urlParamsEqual(params1: URLSearchParams, params2: URLSearchParams): boolean {
