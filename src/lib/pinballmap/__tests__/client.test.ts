...
describe("PinballMapClient", () => {
  ...
  describe("fetchLocationMachineDetails", () => {
    ...
    it("should handle 404 errors gracefully", async () => {
      await expect(client.fetchLocationMachineDetails(999999)).rejects.toThrow(
        PinballMapError,
      );
    });
    ...
    it("should include error details in thrown exception", async () => {
      try {
        ...
        expect(pinballMapError.message).toContain(
  ...
  describe("convenience functions", () => {
    it("should export convenience functions that work with default client", async () => {
      const { fetchLocationMachineDetails, fetchLocationDetails } =
        await import("../client");
