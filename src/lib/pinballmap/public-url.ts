 ...
 * to the pinballmap.com homepage. So this module is the only place a
 * pinballmap.com link may be built; hand-writing one elsewhere is how the
 * requirement gets silently dropped. Catalog-only attribution has no location
 * claim and uses `pinballmapCatalogUrl()` below.
 *
...
/** Attribution target for catalog data when no local PBM location is configured. */
export function pinballmapCatalogUrl(): string {
  return "https://pinballmap.com/";
}
