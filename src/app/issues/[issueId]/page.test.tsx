...
const renderWithProviders = (component: React.ReactElement, session: typeof mockSession | null = null) => {
  const mockTRPCClient = createMockTRPCClient({
    issue: {
      core: {
        getById: jest.fn<Promise<typeof mockIssueData>, [{ id: string }]>(),
        update: jest.fn<Promise<typeof mockIssueData>, [any]>(),
        updateStatus: jest.fn<Promise<typeof mockIssueData>, [any]>(),
        assign: jest.fn<Promise<typeof mockIssueData>, [any]>(),
        close: jest.fn<Promise<typeof mockIssueData>, [any]>(),
      },
      comment: {
...
describe('IssuePage', () => {
  ...
  describe('Public User View', () => {
    it('should display issue details for unauthenticated users', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('issue-description')).toHaveTextContent('Test issue description');
        expect(screen.getByTestId('issue-status')).toHaveTextContent('Open');
        expect(screen.getByTestId('machine-info')).toHaveTextContent('Test Game');
    ...
    it('should hide admin controls for unauthenticated users', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('edit-issue-button')).not.toBeInTheDocument();
        expect(screen.queryByTestId('status-dropdown')).not.toBeInTheDocument();
        expect(screen.queryByTestId('assign-user-button')).not.toBeInTheDocument();
      });
    });
    ...
    it('should show only public comments for unauthenticated users', async () => {
      ...
      const mockTRPCClient = createMockTRPCClient({
        issue: {
          core: {
            getById: jest.fn<Promise<typeof issueWithMixedComments>, [{ id: string }]>()
  ...
  describe('Authenticated User View', () => {
    it('should display full issue details for authenticated users', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('issue-created-by')).toHaveTextContent('Test User');
    ...
    it('should show all comments for authenticated users', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByTestId('internal-comment-badge')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Error States', () => {
    ...
    it('should show network error for failed requests', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Failed to load issue. Please try again.')).toBeInTheDocument();
  ...
  describe('Accessibility', () => {
    it('should have proper ARIA labels and roles', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('main')).toHaveAttribute('aria-label', 'Issue details');
        expect(screen.getByRole('heading', { name: 'Test Issue Title' })).toBeInTheDocument();
        expect(screen.getByRole('region', { name: 'Issue comments' })).toBeInTheDocument();
        expect(screen.getByRole('region', { name: 'Issue timeline' })).toBeInTheDocument();
      });
    });
