...
import { mockIssue, mockUser, mockOrganization } from "~/test/mockContext";
...
const mockAuth = jest.mocked(jest.requireMock("~/server/auth").auth);
const mockDb = jest.requireMock("~/server/db").db;
const mockResolveOrg = jest.mocked(
  jest.requireMock("~/server/api/subdomain").resolveOrganizationFromSubdomain,
);
...
describe("/api/issues/[issueId]", () => {
  ...
  describe("GET /api/issues/[issueId]", () => {
    it("should return issue details for authenticated users", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/issues/test-issue-1",
      );
      const response = await GET(request, {
        params: { issueId: "test-issue-1" },
      });
    ...
    it("should return 404 for non-existent issues", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/issues/non-existent",
      );
      const response = await GET(request, {
    ...
    it("should handle database errors gracefully", async () => {
      ...
      mockDb.issue.findUnique.mockRejectedValue(
  ...
  describe("PUT /api/issues/[issueId]", () => {
    ...
    it("should handle assignment updates", async () => {
      ...
      mockDb.membership.findFirst.mockResolvedValue({
        ...
        organizationId: mockOrganization.id,
        role: {
          permissions: [{ name: "issues:assign" }],
        },
      });
  ...
  describe("DELETE /api/issues/[issueId]", () => {
    it("should delete issue for users with delete permissions", async () => {
      ...
      mockDb.membership.findFirst.mockResolvedValue({
        ...
        organizationId: mockOrganization.id,
        role: {
          permissions: [{ name: "issues:delete" }],
        },
      });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle database constraint violations", async () => {
      ...
      mockDb.issue.findUnique.mockResolvedValue(mockIssue);
      mockDb.issue.update.mockRejectedValue(
    ...
    it("should handle concurrent modification errors", async () => {
      ...
      mockDb.issue.findUnique.mockResolvedValue(mockIssue);
      mockDb.issue.update.mockRejectedValue(
        new Error("Record was updated by another process"),
      );
