...
export function IssueBadgeGrid({
  ...
  return (
    <div className={gridClass} role="group" aria-label="Issue metadata">
      <IssueBadge type="status" value={issue.status} variant={variant} />
      {showPriority && (
        <IssueBadge type="priority" value={issue.priority} variant={variant} />
      )}
      <IssueBadge type="severity" value={issue.severity} variant={variant} />
      <IssueBadge
        type="consistency"
