...
describe("Schema Data Integrity", () => {
  ...
  beforeEach(async () => {
    ...
    await db.insert(schema.users).values([
      {
        id: testUser1Id,
        email: `fk-user-${timestamp}@test.example`,
        name: "FK Test User",
      },
    ]);
  ...
  describe("Organization Relations", () => {
    it("should cascade delete memberships when organization is deleted", async () => {
      ...
      const membershipId = `membership-${timestamp}`;
    ...
    it("should cascade delete all tenant data when organization is deleted", async () => {
      ...
      const priorityId = `priority-cascade-${timestamp}`;
      const statusId = `status-cascade-${timestamp}`;
      ...
      const issueId = `issue-cascade-${timestamp}`;
  ...
  describe("Machine Relations", () => {
    it("should enforce locationId belongs to same organization", async () => {
      ...
      const otherOrgId = `other-org-${timestamp}`;
      const otherLocationId = `other-location-${timestamp}`;

      await db.insert(schema.organizations).values({
        id: otherOrgId,
        name: "Other Organization",
        subdomain: `other-org-${timestamp}`,
      });
    ...
    it("should cascade delete issues when machine is deleted", async () => {
      ...
      const issueId = `issue-machine-${timestamp}`;
      await db.insert(schema.issues).values({
        id: issueId,
        title: "Machine Issue",
        organizationId: seededOrgId,
  ...
  describe("User & Membership Relations", () => {
    it("should allow users to have memberships in multiple organizations", async () => {
      ...
      const org2Id = `test-org2-${timestamp}`;
      await db.insert(schema.organizations).values({
        id: org2Id,
      ...
      const role1Id = `role1-${timestamp}`;
      const role2Id = `role2-${timestamp}`;
      ...
      const membership1Id = `membership1-${timestamp}`;
      const membership2Id = `membership2-${timestamp}`;
