  ...
  7: {
    ...
    patterns: {
      required: [
        "withRLSAwareTest",
        "testSessions",
        "sql`SET app.current_organization_id",
      ],
...
export function detectArchetype(
  ...
  const patterns = {
    ...
    2: () =>
      content.includes("Service") && content.includes("withBusinessLogicTest"),
    3: () =>
      content.includes("integration.test.ts") ||
      (content.includes("withBusinessLogicTest") &&
        content.includes("schema.")),
    4: () =>
      content.includes("render(") && content.includes("@testing-library/react"),
    ...
    8: () =>
      ...
      (content.includes("schema") && content.includes("foreign")),
  };
...
function validateMemorySafety(
  ...
  const dangerousPatterns = [
    {
      ...
      message:
        "CRITICAL: Per-test PGlite instances cause memory blowouts and system lockups",
      suggestion:
        "Use worker-scoped PGlite with withBusinessLogicTest helper instead",
    },
    {
      pattern: /createSeededTestDatabase\(\)/g,
      message:
        "CRITICAL: Per-test database creation causes 50-100MB memory usage per test",
      suggestion:
        "Use withBusinessLogicTest with worker-scoped database pattern",
    },
    {
      pattern: /beforeEach.*createTestDatabase/g,
      message:
        "CRITICAL: beforeEach database creation multiplies memory usage by test count",
      suggestion: "Use worker-scoped fixtures with test.extend pattern",
    },
    {
      pattern: /beforeAll.*new PGlite/g,
      message:
        "HIGH: Multiple PGlite instances across test files cause memory issues",
  ...
  if (
    ...
    result.warnings.push({
      type: "performance",
      message:
        "Integration test should use withBusinessLogicTest or withRLSAwareTest helpers",
      suggestion:
        "Import and use proper test isolation helpers for memory safety",
    });
  }
}
...
function validateRLSContext(
  ...
  const hasRLSPattern = rlsPatterns.some((pattern) =>
  ...
  if (!hasRLSPattern) {
    result.errors.push({
      ...
      suggestion:
        "Use testSessions helpers or withRLSAwareTest for organizational scoping",
    });
  ...
  if (content.includes("organizationId") && !content.includes("cross-org")) {
    result.suggestions.push(
      "Consider adding cross-organizational isolation tests",
    );
  }
}
...
function validateArchetypeCompliance(
  content: string,
  filePath: string,
  result: ValidationResult,
): void {
  ...
  for (const pattern of archetype.patterns.required) {
    if (!content.includes(pattern)) {
      result.errors.push({
        ...
        severity: "medium",
        message: `Missing required pattern for ${String(archetype.name)}: ${String(pattern)}`,
        suggestion: `Add ${String(pattern)} to follow ${String(archetype.name)} archetype`,
      });
  ...
  for (const pattern of archetype.patterns.forbidden) {
    if (content.includes(pattern)) {
      result.errors.push({
        type: "archetype_mismatch",
        severity: "medium",
        message: `Forbidden pattern in ${String(archetype.name)}: ${String(pattern)}`,
        suggestion: `Remove ${String(pattern)} to comply with ${String(archetype.name)} archetype`,
      });
      result.compliance.archetypePattern = false;
    }
  }
...
function validateImportStructure(
    ...
    for (const forbidden of archetype.imports.forbidden) {
      if (line.includes(forbidden)) {
        result.errors.push({
          ...
          severity: "medium",
          message: `Forbidden import for ${String(archetype.name)}: ${String(forbidden)}`,
          suggestion: `Remove import of ${String(forbidden)} - not needed for this archetype`,
  ...
  if (!hasRequiredImports) {
    result.warnings.push({
      ...
      message: `Consider importing from: ${String(archetype.imports.allowed.join(", "))}`,
...
function validateErrorHandling(
  ...
  const hasExpectRejects =
    content.includes("expect(") && content.includes(".rejects.toThrow");
  const hasExpectThrows =
    content.includes("expect(() =>") && content.includes(").toThrow");
  ...
  if (
    ...
    result.warnings.push({
      ...
      suggestion:
        "Consider adding tests for error scenarios with expect().rejects.toThrow() or try/catch",
    });
  }
...
function validatePerformancePatterns(
  content: string,
  filePath: string,
  result: ValidationResult,
): void {
  ...
  const performanceIssues = [
    {
      ...
      message:
        "Performance issue: forEach with async/await doesn't run in parallel",
    ...
    },
    {
      ...
      suggestion:
        "Move expensive setup to beforeAll or use lazy initialization",
    },
  ];
...
export function generateValidationReport(
  ...
  const report = [
    "=".repeat(80),
    "TEST ARCHETYPE VALIDATION REPORT",
    "=".repeat(80),
    "",
  ];
  ...
  for (const [filePath, result] of results) {
    ...
    if (!result.isValid || result.warnings.length > 0) {
      ...
      report.push(
        `   Archetype: ${result.archetype ? String(ARCHETYPE_DEFINITIONS[result.archetype].name) : "Unknown"}`,
      );
  ...
  report.push("SUMMARY");
  report.push("=".repeat(80));
  report.push(`Total files validated: ${String(totalFiles)}`);
  report.push(
    `Valid files: ${String(validFiles)} (${String(Math.round((validFiles / totalFiles) * 100))}%)`,
  );
  report.push(`Critical errors: ${String(criticalErrors)}`);
  report.push(`High priority errors: ${String(highErrors)}`);
  report.push(`Warnings: ${String(warnings)}`);
  report.push("");

  if (criticalErrors > 0) {
    report.push(
      "🔥 CRITICAL ISSUES DETECTED - Fix immediately to prevent system lockups",
    );
