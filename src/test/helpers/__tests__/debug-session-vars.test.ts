...
test("Debug session variables step by step", async () => {
  ...
  await db.execute(sql.raw(`SELECT set_config('app.test_value', 'hello', false)`));

  // Try to read it back
  const result1 = await db.execute(sql`SELECT current_setting('app.test_value', true) as value`);
  ...
  // Try to read it back
  const result2 = await db.execute(sql`SELECT current_setting('app.test_value2', true) as value`);
