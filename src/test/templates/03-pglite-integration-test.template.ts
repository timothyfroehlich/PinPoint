...
describe("YourModule Integration Tests", () => {
  ...
  test("creates and retrieves data with proper relationships", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      expect(issueWithRelations?.organization.name).toBe("Integration Test Organization");
      expect(issueWithRelations?.createdBy.email).toBe("integration@test.com");
    });
  });
  
  test("handles complex queries with joins and aggregations", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      const complexQuery = await db
        ...
        .leftJoin(schema.issues, eq(schema.machines.id, schema.issues.machineId))
        .leftJoin(schema.organizations, eq(schema.machines.organizationId, schema.organizations.id))
        .where(eq(schema.organizations.id, testData.organizationId))
        .groupBy(schema.machines.id, schema.machines.name, schema.organizations.name)
      ...
      expect(complexQuery.every(row => row.organizationName === "Complex Test Org")).toBe(true);
    });
  });
  ...
  test("handles transactions correctly", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      const results = await db.transaction(async (tx) => {
        // Create machine
        const [machine] = await tx.insert(schema.machines).values({
        ...
        const issues = await tx.insert(schema.issues).values([
          {
            title: "Issue 1",
            machineId: machine.id,
            organizationId,
  ...
  test("handles large dataset queries efficiently", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      const machines = await db.insert(schema.machines).values(machineData).returning();
...
async function createBasicTestData(db: any) {
  const [organization] = await db.insert(schema.organizations).values({
    id: `basic-org-${Date.now()}`,
    name: "Basic Test Organization",
  }).returning();
  
  const [user] = await db.insert(schema.users).values({
    id: `basic-user-${Date.now()}`,
...
async function createComplexTestDataset(db: any) {
  ...
  const issues = await db.insert(schema.issues).values([
    {
