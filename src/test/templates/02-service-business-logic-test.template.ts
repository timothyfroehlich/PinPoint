...
describe("YourService Business Logic", () => {
  ...
  test("handles complex workflow correctly", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      const [initialResource] = await db
        .insert(schema.yourTable)
        .values({
          id: SEED_TEST_IDS.MOCK_PATTERNS.ISSUE,
  ...
  test("handles resource not found gracefully", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      await expect(service.getResourceById("nonexistent-id")).rejects.toThrow(
        "Resource not found",
      );
  ...
  test("bulk operations perform efficiently", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      expect(results.every((r) => r.organizationId === organizationId)).toBe(
        true,
      );
    });
  });
...
4. Update SEED_TEST_IDS references to match your test constants.
   - Always use static SEED_TEST_IDS constants instead of dynamic queries.
   - Example: `const orgId = SEED_TEST_IDS.ORGANIZATIONS.primary;`
5. Customize business logic tests for your specific service
6. Update helper functions for your data requirements
...
- This template demonstrates the correct static constant approach.
- No dynamic database queries needed - use SEED_TEST_IDS directly.
- Uses SEED_TEST_IDS for predictable business logic testing
- Mock patterns provide consistent template IDs for unit tests
- Static IDs enable deterministic business rule validation
- Avoids dynamic ID generation for reproducible service tests
*/
