...
describe("Database Schema and Constraints", () => {
  ...
  test("enforces foreign key constraints on issue creation", async ({
    workerDb,
  }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      // ARRANGE: Use seeded organization for constraint tests
  ...
  test("enforces cascading deletes correctly", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      // ARRANGE: Use seeded organization for cascading delete tests
      const orgId = SEED_TEST_IDS.ORGANIZATIONS.primary;
      const org = await db.query.organizations.findFirst({
        where: eq(schema.organizations.id, orgId),
      });
  ...
  test("enforces unique constraints on machine serial numbers", async ({
    workerDb,
  }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
  ...
  test("enforces unique constraints within organizational scope", async ({
    workerDb,
  }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
  ...
  test("enforces NOT NULL constraints on required fields", async ({
    workerDb,
  }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      // ARRANGE: Setup organization
  ...
  test("RLS policies work with foreign key constraints", async ({
    workerDb,
    organizationId,
  }) => {
  ...
  test("unique constraints respect organizational boundaries", async ({
    workerDb,
  }) => {
  ...
  test("database triggers maintain data consistency", async ({ workerDb }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
      ...
      expect(issue.createdAt.getTime()).toBeLessThanOrEqual(
  ...
  test("provides meaningful constraint violation errors", async ({
    workerDb,
  }) => {
    await withBusinessLogicTest(workerDb, async (db) => {
