...
describe("Archetype Integration: RSC Test System Working Together", () => {
  ...
  describe("Archetype 2: Service Business Logic Tests", () => {
    ...
    it("simulates DAL function with mock auth context", async () => {
      ...
      const getIssuesForOrg = async () => {
        ...
        return allIssues.filter(issue => issue.organization_id === organizationId);
      };
  ...
  describe("Archetype 3: Server Action Integration", () => {
    it("demonstrates complete Server Action flow with mocks", async () => {
      ...
      const mockGetAuthContext = vi.fn().mockResolvedValue(scenario.authContext);
      ...
      const createIssueFlow = async (formData: FormData) => {
        try {
          ...
          return actionError(error instanceof Error ? error.message : "Unknown error");
        }
      };
    ...
    it("handles validation errors in complete flow", async () => {
      const mockGetAuthContext = vi.fn().mockResolvedValue(
        MockAuthContextFactory.createPrimaryOrgContext()
      );
  ...
  describe("Cross-Archetype Integration", () => {
    it("demonstrates data flow from FormData to DAL to database", async () => {
      ...
      expect(authContext.organizationId).toBe(SEED_TEST_IDS.ORGANIZATIONS.primary);
      ...
      expect(issueData.organizationId).toBe(SEED_TEST_IDS.ORGANIZATIONS.primary);
    ...
    it("demonstrates cross-org security at every layer", async () => {
      const securityScenario = MockScenarioFactory.createCrossOrgSecurityScenario();
      ...
      expect(primaryAuth.organizationId).toBe(SEED_TEST_IDS.ORGANIZATIONS.primary);
      ...
      expect(competitorMachine.organization_id).toBe(SEED_TEST_IDS.ORGANIZATIONS.competitor);
      expect(attemptedIssue.organizationId).toBe(SEED_TEST_IDS.ORGANIZATIONS.primary);
      expect(competitorMachine.organization_id).not.toBe(attemptedIssue.organizationId);
  ...
  describe("Performance and Reliability Patterns", () => {
    it("demonstrates efficient mock data generation for large datasets", () => {
      ...
      const scenario = MockScenarioFactory.createEdgeCaseScenarios().highVolumeScenario;
    ...
    it("demonstrates memory-efficient testing patterns", () => {
      ...
      expect(scenarios.every(s => s.organization.id === firstScenarioOrgId)).toBe(true);
