...
export const createIssueStates = {
  new: (overrides: Record<string, any> = {}) => createIssueFactory({
    overrides: {
      statusId: 'status-new',
      status: createStatusFactory({ overrides: { id: 'status-new', name: 'New' } }),
      assignedToId: null,
      assignedTo: null,
      ...overrides,
    },
  ...
  acknowledged: (overrides: Record<string, any> = {}) => createIssueFactory({
    overrides: {
      statusId: 'status-acknowledged',
      status: createStatusFactory({ overrides: { id: 'status-acknowledged', name: 'Acknowledged' } }),
      assignedToId: 'user-technician',
      assignedTo: createUserFactory({ overrides: { id: 'user-technician', name: 'Tech User' } }),
      ...overrides,
    },
  }),
  
  inProgress: (overrides: Record<string, any> = {}) => createIssueFactory({
    overrides: {
      statusId: 'status-in-progress',
      status: createStatusFactory({ overrides: { id: 'status-in-progress', name: 'In Progress' } }),
      assignedToId: 'user-technician',
      assignedTo: createUserFactory({ overrides: { id: 'user-technician', name: 'Tech User' } }),
      ...overrides,
    },
  ...
  resolved: (overrides: Record<string, any> = {}) => createIssueFactory({
    overrides: {
      statusId: 'status-resolved',
      status: createStatusFactory({ overrides: { id: 'status-resolved', name: 'Resolved' } }),
...
export const createCommentTypes = {
  public: (overrides: Record<string, any> = {}) => createCommentFactory({
    overrides: {
      isInternal: false,
  ...
  internal: (overrides: Record<string, any> = {}) => createCommentFactory({
    overrides: {
      isInternal: true,
      content: `Internal note: ${faker.lorem.sentence()}`,
      createdById: 'user-technician',
      createdBy: createUserFactory({ overrides: { id: 'user-technician', name: 'Tech User' } }),
      ...overrides,
    },
  }),
  
  resolution: (overrides: Record<string, any> = {}) => createCommentFactory({
    overrides: {
      isInternal: false,
...
export const createUserRoles = {
  admin: (overrides: Record<string, any> = {}) => createUserFactory({
    overrides: {
      name: 'Admin User',
      email: 'admin@test.com',
      permissions: ['issues:read', 'issues:edit', 'issues:delete', 'issues:assign', 'issues:close', 'issues:comment', 'issues:internal_comment'],
      role: createRoleFactory({ overrides: { name: 'Admin' } }),
      ...overrides,
    },
  }),
  
  technician: (overrides: Record<string, any> = {}) => createUserFactory({
    overrides: {
      name: 'Technician User',
      email: 'technician@test.com',
      permissions: ['issues:read', 'issues:edit', 'issues:assign', 'issues:close', 'issues:comment', 'issues:internal_comment'],
      role: createRoleFactory({ overrides: { name: 'Technician' } }),
      ...overrides,
    },
  }),
  
  member: (overrides: Record<string, any> = {}) => createUserFactory({
    overrides: {
      name: 'Member User',
  ...
  owner: (overrides: Record<string, any> = {}) => createUserFactory({
    overrides: {
      name: 'Machine Owner',
      email: 'owner@test.com',
      permissions: ['issues:read', 'issues:comment', 'issues:priority_request'],
...
export const createModelFactory = (options: FactoryOptions = {}) => {
  ...
  return {
    ...
    manufacturer: faker.helpers.arrayElement(['Stern', 'Williams', 'Gottlieb', 'Bally', 'Data East', 'Sega']),
...
export const createStatusFactory = (options: FactoryOptions = {}) => {
  ...
  return {
    id: faker.string.uuid(),
    name: faker.helpers.arrayElement(['New', 'Acknowledged', 'In Progress', 'Resolved', 'Closed']),
...
export const createRoleFactory = (options: FactoryOptions = {}) => {
  ...
  return {
    id: faker.string.uuid(),
    name: faker.helpers.arrayElement(['Admin', 'Technician', 'Member', 'Owner']),
    organizationId: 'org-test',
    createdAt: faker.date.past({ years: 1 }),
    updatedAt: faker.date.recent({ days: 30 }),
...
export const createAttachmentFactory = (options: FactoryOptions = {}) => {
  ...
  const createSingleAttachment = (customOverrides: Record<string, any> = {}) => ({
    id: faker.string.uuid(),
...
export const createActivityFactory = (options: FactoryOptions = {}) => {
  ...
  const createSingleActivity = (customOverrides: Record<string, any> = {}) => ({
    id: faker.string.uuid(),
    type: faker.helpers.arrayElement(['status_change', 'assignment', 'comment', 'priority_change', 'attachment']),
