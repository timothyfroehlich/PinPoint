...
import { createMockContext, type MockContext } from "../mockContext";
...
describe("Golden Example: Schema-Validated Testing", () => {
  ...
  describe("Issue Operations with Schema Validation", () => {
    it("should create issue with proper relationships", async () => {
      ...
      const mockIssue: Issue = IssueSchema.parse({
        ...
        statusId: "status_123",
        priorityId: "priority_123",
        createdById: "usr_123abc",
        assignedToId: null,
      });

      // ✅ Validate input data structure
      const issueInput = IssueCreateInputSchema.parse({
        title: "Flipper not working",
        description: "Left flipper is unresponsive",
        consistency: "Always",
      });
      ...
      // Execute operations
      const machine = await ctx.db.machine.findUnique({ where: { id: "machine_123" } });
  ...
  describe("Schema Utility Functions", () => {
    it("should provide type-safe mock data generation helpers", () => {
      ...
      const user2 = createValidUser({ name: "Custom User", email: "custom@example.com" });
...
export { UserSchema, UserCreateInputSchema, IssueSchema, IssueCreateInputSchema };