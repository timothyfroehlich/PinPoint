...
describe("Dual-Track Testing Examples", () => {
  ...
  describe("Track 2A: Fast Business Logic Testing", () => {
    ...
    test("Issue priority calculation - FAST business logic focus", async ({ workerDb }) => {
      await withBusinessLogicTest(workerDb, async (db) => {
        ...
        // Direct data creation (no RLS overhead)
        const [org] = await db.insert(schema.organizations).values({
          id: "test-org-business",
    ...
    test("Complex workflow - Multi-step business process", async ({ workerDb }) => {
      await withBusinessLogicTest(workerDb, async (db) => {
        // ✅ PERFECT FOR: Complex business workflows where security isn't the focus
        
        // Setup test data quickly (no RLS coordination)
        const [org] = await db.insert(schema.organizations).values({
          id: "workflow-org",
        ...
        const [initialIssue] = await db.insert(schema.issues).values({
        ...
        const escalatedIssue = await escalateIssueIfNeeded(db, initialIssue.id, {
  ...
  describe("Track 2B: RLS-Aware Integration Testing", () => {
    ...
    test("Business logic WITH organizational context", async ({ workerDb, organizationId }) => {
      await withRLSAwareTest(workerDb, organizationId, async (db) => {
        ...
        expect(allOrgIssues.every(i => i.organizationId === organizationId)).toBe(true);
      });
    });
    
    test("RLS-dependent business logic edge case", async ({ workerDb, organizationId }) => {
      await withRLSAwareTest(workerDb, organizationId, async (db) => {
        ...
        expect(machineIssues.every(issue => issue.organizationId === organizationId)).toBe(true);
        ...
        expect(machineWithIssues?.issues.every(i => i.organizationId === organizationId)).toBe(true);
      });
    });
  });
  ...
  describe("Track 2C: Cross-Organizational Boundary Testing", () => {
    ...
    test("Business logic respects organizational boundaries", async ({ workerDb }) => {
      const orgContexts = [
  ...
  describe("Track Selection Guide", () => {
    ...
    test("Decision matrix examples", async ({ workerDb, organizationId }) => {
      ...
      await withCrossOrgTest(workerDb, contexts, async (setContext, db) => {
        ...
        expect(report.data.every(item => item.organizationId === "test-org-1")).toBe(true);
      });
    });
  });
});
...
async function escalateIssueIfNeeded(
  db: any, 
  issueId: string, 
  metrics: { downtime: number; customerComplaints: number; revenueImpact: number }
) {
...
function calculateMaintenanceSchedule(params: {
  ...
  const urgencyLevel = daysSinceLastMaintenance > 180 ? "high" : 
                      daysSinceLastMaintenance > 90 ? "medium" : "low";
  
  const nextMaintenance = new Date();
  nextMaintenance.setDate(nextMaintenance.getDate() + (urgencyLevel === "high" ? 7 : 30));
...
function getTopMachinesByIssues(issues: any[]) {
  const machineCounts = issues.reduce((acc, issue) => {
