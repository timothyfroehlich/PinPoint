...
- **Config files**: `supabase/config.toml` and `.env.local` are auto-generated by the `post-checkout` hook (`scripts/worktree_setup.py`) and are **read-only**.
...
3. Re-run `python3 scripts/worktree_setup.py` — regenerate config if mismatched
