...
- Admin Discord integration test sends a real test request through the dependency-injected channel.

### NOT OK to E2E (simulating third-party internals)

- Pre-seed an `auth.identities` row via raw SQL to test "Discord linked" UI state. Invalidates the GoTrue session on next middleware refresh. Casework: **PP-e20** (resolved by deleting the spec — PR #1296).
- Bypass Turnstile in test mode by patching the SDK to return success. The provider validates upstream of our code. Casework: **PP-uc8** (resolved by commenting out captcha keys in `.env.local` — PR #1283).
- Regex-extract a password-reset link from a Supabase test-email. The format is GoTrue's, varies by version, breaks silently on upgrade. Casework: **PP-q9r** (pending deletion).
