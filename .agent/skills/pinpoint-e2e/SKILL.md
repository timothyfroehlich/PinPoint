...
2. **Does an integration or unit test already cover this feature?** Run `rg -l <keyword> src/ src/test/ tests/` first. The audit found that agents create new specs because they can't see the existing tests — most of the time you should be extending an existing file, not creating a new one. See `pinpoint-testing` skill § "Where Existing Coverage Lives".
