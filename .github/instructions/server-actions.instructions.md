...
export async function createIssue(formData: FormData): Promise<{ ok: boolean; error?: string }> {
  const raw = {
...
export default async function NewIssuePage({ machineId }: { machineId: string }) {
  return (
    <form action={createIssue} className="space-y-4">
      ...
      <button type="submit" className="btn">Create Issue</button>
    </form>
  );
}
```
