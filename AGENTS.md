...
**Never** invoke `pnpm exec playwright test` with no spec path — it runs every spec in one Playwright process and cross-contaminates seed state. The full suite (`e2e:full` / `e2e:all`) is CI's job by default — roughly 8–10 minutes of three parallel workers plus a Supabase stack and a Next server, peaking at several GB. Run it locally when you actually want the signal and the host has the headroom. **Always pass `--project=chromium`** — for targeted runs, and for `e2e:full` too. Leaving it off runs all four browser projects concurrently against one database, and several specs write singleton seeded rows (the member's own profile, machine owners, machine settings), so the projects interleave and each asserts another's value. That produces red runs that are not bugs. `e2e:all` already pins chromium for this reason; cross-browser coverage belongs to CI, where each browser gets its own job and its own database. Use `--headed` to debug visually. Report flaky tests; don't retry in a loop.
