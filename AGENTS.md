...
### PinPoint Beads Overrides

The generated setup above is generic. PinPoint uses the shared Dolt server configured by the primary checkout's `.beads/metadata.json`; run `bd context` to verify the active store. Treat Dolt sync and repository-changing operations as policy-controlled handoffs. Codex mutations must use `bd --actor Codex <command>` so audit history never falls back to Tim's identity.
