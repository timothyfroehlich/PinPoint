...
Those three are now **mechanically enforced**, not just documented: `pkill`, `killall`, and `supabase stop --all` sit on `permissions.deny` in the checked-in `.claude/settings.json`, alongside `shutdown` / `reboot` / `mkfs` and the repo-destructive `gh issue delete` / `gh repo delete`. `permissions.ask` carries the rest of the destructive set — `rm -rf`, `docker rm`, `docker volume rm`, `kill`, and the history-rewriting git verbs (`rebase`, `reset --hard`, `clean -f`, `branch -D`, every `push --force` spelling).

They live in the **project** file rather than user settings because that is the only portable channel: `permissions.allow`/`deny`/`ask` concatenate and dedupe across every settings scope, so a rule checked in here reaches both the Mac and Bazzite through `git pull`, while `~/.claude/settings.json` is machine-local by design (it holds absolute `$HOME` paths and per-OS config, and Claude Code has no user-scope `settings.local.json` and no `extends` to split it). The same duplication is deliberate: these rules also remain in the Mac's user settings so they still apply in every non-PinPoint repo. Everything CORE-SEC-010 says about scope applies here too — prefix matchers over the command string, a speed bump against accidents, not a security boundary, and they don't reach child processes.
