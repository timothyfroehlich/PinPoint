...
| `pnpm run preflight`                         | Full: static checks + unit + DB reset + build + integration + smoke. **For non-trivial changes** (migrations, auth, server actions, middleware, DB schema) — not every commit. Host-wide cap of 2 concurrent runs (via `sem`); use `preflight:unlocked` to bypass.                                                                 |
...
| `pr-lifecycle-watcher` agent                 | Wait on CI or review for a PR head (`pinpoint-pr-workflow` Phase 3). Direct `pr-watch.py` watches are hook-blocked; `pr-watch.py <PR> --check-ready` is allowed. Never hand-roll a polling loop.                                                                                                                                   |
