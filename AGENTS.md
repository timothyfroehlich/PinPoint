...
- **Create**: `git worktree add --no-track -b branch /path origin/main` — the hook handles the rest. `--no-track` keeps `origin/main` from becoming the branch's upstream; `git push -u origin branch` sets the real one.
- **Cleanup**: `python3 scripts/worktree_cleanup.py <worktree-path>` is the complete teardown command: it stops the pinned Supabase project, removes its volumes, unlocks/removes/prunes the Git worktree, then releases its slot. Claude's `WorktreeRemove` hook calls the same module via `--claude-hook`; configure Codex cleanup as `python3 scripts/worktree_cleanup.py .`. Plain `git worktree remove` or `rm -rf` bypasses it and leaks resources; `python3 scripts/worktree_reap.py --apply` reconciles those.
- **Reaping**: `scripts/worktree_reap.py` is the one reclaim command. It removes worktrees whose work already landed, delegating teardown to `worktree_cleanup.py`, and reclaims what deleted worktrees left behind: slot entries and Supabase containers, networks and volumes (on the remote daemon too when the remote backend is in use). It consumes Git's complete worktree inventory, regardless of whether Claude, Codex, Antigravity, or a human chose the path. Reap requires positive proof: a merged PR whose `headRefOid` **is** local `HEAD` with a clean tree, or a clean worktree at least a day old with zero commits ahead of `origin/main`. Dirty tree, post-merge commits, an open PR, or unreachable `gh` means "leave it alone", and any `gh` or Docker state it cannot read is reported as UNKNOWN, never zero. Dry-run by default; `--apply` to reclaim. `merge-pr.sh` reaps the merged branch's worktree automatically. (PP-49x5.)
- **SessionStart audit**: one hook runs `worktree_reap.py --quiet` (dry-run) every 6h and prints a one-line nudge when something is reclaimable or UNKNOWN.
- **Ports**: main worktree uses defaults (3000 / 54321 / 54322). Slot N: `3000+N*10`, `54321+N*100`, `54322+N*100`. CI remaps the Supabase ports to 24xxx (below the runner's ephemeral port range) in `.github/actions/setup-supabase`; local ports are unaffected. (PP-3w32.2.)
- **Supabase `project_id`**: derived from the branch name (a detached HEAD uses the worktree directory name plus a hash of its path) the **first** time a worktree is set up, then **pinned** — later checkouts reuse the id already in `supabase/config.toml`. It names the Docker containers and labels the volumes, so letting it follow a `git checkout -b` would rename the stack out from under itself (`supabase stop` matches nothing, the old containers keep the ports bound, cleanup misses them). `config.toml` is the authoritative record of the running stack's id. (PP-4936.)
