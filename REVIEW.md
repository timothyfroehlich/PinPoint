...
The Claude Code `block-direct-merge.cjs` hook adds an approval prompt before the guarded script runs. Codex, Antigravity, and Gemini lack that hook; Tim’s explicit request in the task is their authorization to pass `--human`, not a reason to refuse or ask him to type a command. If the target is ambiguous, clarify it first. If any gate fails, stop and report it; bypass flags need separate explicit authorization.
