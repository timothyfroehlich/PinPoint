...
export async function getCurrentSession(db: TestDatabase): Promise<{
  ...
  try {
    ...
    const [roleResult] = await db.execute(
      sql`SELECT current_setting('app.current_user_role', true) as value`,
    );
    const [envResult] = await db.execute(
      sql`SELECT current_setting('app.environment', true) as value`,
    );

    return {
      organizationId: orgResult?.value || null,
      userId: userResult?.value || null,
      role: roleResult?.value || null,
      environment: envResult?.value || null,
    };
  } catch {
    return {
      organizationId: null,
...
export class MultiContextManager {
  ...
  async switchTo(contextId: string): Promise<void> {
    ...
    if (!context) {
      throw new Error(
        `Context '${contextId}' not found. Available contexts: ${this.contexts.map((c) => c.id).join(", ")}`,
      );
    }
...
export const sessionPatterns = {
  ...
  async withMultipleRoles<T>(
    db: TestDatabase,
    orgId: string,
    roles: (keyof typeof testSessions)[],
    ...
    for (const role of roles) {
      if (
        typeof testSessions[role] === "function" &&
        role !== "switchOrganization" &&
        role !== "impersonate"
      ) {
  ...
  async validateSetup(db: TestDatabase): Promise<{
    ...
    if (!session.organizationId) {
      ...
      recommendations.push(
        "Set organization context with testSessions.admin() or similar",
      );
    }
