...
export async function withRLSContext<T>(
  ...
  await db.execute(
    sql.raw(`SET app.current_user_id = '${userId.replace(/'/g, "''")}'`),
  );
  await db.execute(
    sql.raw(
      `SET app.current_organization_id = '${organizationId.replace(/'/g, "''")}'`,
    ),
  );
  await db.execute(
    sql.raw(`SET app.current_user_role = '${role.replace(/'/g, "''")}'`),
  );
  await db.execute(
    sql.raw(`SET app.current_user_email = '${email.replace(/'/g, "''")}'`),
  );
...
export async function withAdminContext<T>(
  ...
  const organizationId =
    options?.organizationId ?? SEED_TEST_IDS.ORGANIZATIONS.primary;
