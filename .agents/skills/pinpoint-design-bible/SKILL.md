...
- **Overflow testing:** Every page must pass `assertNoHorizontalOverflow()` in its smoke test, at both mobile (375px) and desktop (1024px) viewports. Add new pages to `e2e/smoke/responsive-overflow.spec.ts`. It does **not** compare `scrollWidth` to `clientWidth` — the app shell sets `overflow-x: hidden`, so an overrun is silently clipped rather than widening the document and that check can never fail. It looks for content a non-scrollable ancestor has clipped off-screen, while treating real scroll containers (carousels, tab strips) as fine. Read `e2e/support/actions.ts` before changing what a page does about overflow; optimising for the naive check is optimising for a test that cannot fail.
