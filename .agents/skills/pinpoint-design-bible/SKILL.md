...
description: >-
  ...
  responsive strategy, and player-centric issue severity vocabulary (cosmetic,
  minor, major, unplayable). Governs browser-support Baseline policy,
  form-correctness conventions, picker patterns, shell contracts (fixed header,
  tab-bar, z-index hierarchy), semantic color tokens, and telegraphic copy
  standards. Use when building new UI, pages, or components; when styling or
  writing copy for issue severity, status lines, or disabled controls; when
  positioning against the app shell; or when writing forms and pickers.
---
