...
- **Target, not current behaviour:** when opening any modal, set `inert` on the page-root container (CORE-A11Y-006). Radix already applies `aria-hidden` + pointer-events to the background; `inert` is the platform primitive that also removes it from tab order. **Nothing implements this yet** — it's adopted and tracked as PP-kqbk.8, so an empty search for `inert` means "not built", not "decided against". Don't add a partial version per modal; it goes on the background container once. Detail: `pinpoint-ui` skill → `references/accessibility.md`.
- Native `<dialog>.showModal()` is Baseline Widely available, but it is **not** the default for product UI — shadcn `<Dialog>` / `<AlertDialog>` / `<Sheet>` are. Reach for `<dialog>` only for one-off, self-contained, single-purpose surfaces that don't earn a place in the shadcn variant system (e.g., a debug-only inspector, a tightly-scoped help blurb). See `pinpoint-ui` skill § "Native HTML primitives alongside shadcn/Radix".
