...
description: >-
  ...
  including the local/preview environment defaults — dev autologin
  (`DEV_AUTOLOGIN_ENABLED`) and its three opt-outs (`x-skip-autologin` header,
  `skip_autologin` cookie, `?autologin=off`), the global-setup
  preflight→migrate→fast-reset→full-reset chain and the `SKIP_SUPABASE_RESET`
  bypass, and the `/api/test-data/cleanup` endpoint (`issueIds` / `machineIds` /
  `issueTitlePrefix`). Carries the technique `pinpoint-testing` defers here: the
  30-second pre-flight that routes a bug class away from E2E before you write the
  spec, the selector strategy (roles and labels first, testids next, CSS never),
  the auth roles and `loginAs` / `STORAGE_STATE` scaffolding, new-spec scaffolds,
  and the crosstalk/session/timeout debugging checklist. Use when writing,
  debugging, or fixing E2E tests to ensure worker isolation and stability, when
  choosing a selector or a logged-in role for a spec, when a test is
  unexpectedly authenticated or needs to run as a guest, or when E2E seed/cleanup
  state looks wrong.
---
