...
- **Tests** — `src/test/integration/transaction-tripwire.test.ts` (the runtime tripwire) and `src/test/eslint/no-side-effects-in-transaction.test.ts` (the static rule).
...
- **Never log PII — log user IDs instead.** The `redact` list is a backstop for a fixed set of key names, not a filter. `EMAIL_LOG_KEYS` (`email`, `reporterEmail`, `userEmail`, `contactEmail`, `submittedEmail`) is crossed with wildcard depths 0–3, so those five names are caught from the top level of the payload down to four levels in, array indices included. Anything else — a differently-named key, or one of these nested deeper still — is logged raw, to `logs/<session>/app.log` locally and to stdout on Vercel. Pino has no recursive wildcard, so the depth is a bounded choice rather than a guarantee; mask at the call site with `maskEmail` (`~/lib/logging/mask`) and treat the list as the safety net. (PP-tg9y widened it from the original two keys at one level.)
