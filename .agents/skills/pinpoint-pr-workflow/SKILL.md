...
Requires a dev server and Supabase. Its first auth setup can reset + reseed the database, so use an explicitly local stack for screenshots on Tim's Mac: stop this worktree's remote stack, start local Supabase, and run the dev server with `pnpm run dev:local` plus `PINPOINT_SUPABASE_BACKEND=local` for the screenshot command. The remote-backed development database is persistent and `e2e/global-setup.ts` refuses to reset it. See `docs/runbooks/remote-supabase.md`.
