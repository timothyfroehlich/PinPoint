...
**What the rule does not do is make the attestation true.** It removes the harness's opinion about whether you earned the marker. An incorrect marker can make the gate claim a review that never happened. In Claude Code, the merge hook prompts Tim again; in other harnesses, his direct merge request is the authorization. Neither changes the requirement that the review evidence be honest and exact-head.
...
**Tim decides whether to merge.** If he explicitly requests it in the active task (for example, “merge it” with one unambiguous PR), the owning agent in any harness runs `bash scripts/workflow/merge-pr.sh <PR> --human`. The flag records Tim’s authorization; he need not repeat it in a shell. The script rechecks CI, exact-head review, unresolved threads, and conflicts. Claude Code additionally prompts through `block-direct-merge.cjs`; Codex, Antigravity, and Gemini lack that prompt, so the direct user request is their authorization. If the PR target is ambiguous, clarify it. If a gate fails, stop and report the blocker. Raw channels (`gh pr merge`, `gh api PUT .../merge`, MCP `merge_pull_request`) remain prohibited for agents because they skip the gates. This applies to implicit current-repository and explicit `timothyfroehlich/PinPoint` targets; statically explicit non-PinPoint targets follow their own policy. Environment-only selectors fail closed.
