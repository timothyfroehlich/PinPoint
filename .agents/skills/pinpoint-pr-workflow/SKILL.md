...
> **Step 3 is the load-bearing control, not a formality.** The three preconditions the script enforces are metadata checks. The thing actually standing between a tampered dependency bump and production is **you reading the diff**.
>
> `mark-claude-review.sh` is an **honesty contract**: it attests that _you personally read the diff at that exact head SHA_. It verifies nothing. **Never post it for a diff you did not read.** It is SHA-pinned, so any new commit — including a `@dependabot rebase` — voids it and you must read and attest again.
>
> **What you are looking for: anything that is not a dependency bump.** A `.github/workflows/**` edit above all. That path is _necessarily_ allowlisted, because Dependabot bumps pinned action SHAs there — so a workflow change riding along inside a "dependency bump" is CI execution with repo secrets. That is the attack this gate exists to catch.

The `reviewed` gate is deliberately _not_ waived for Dependabot PRs: GitHub never issues a Copilot review for them, and `--dependabot` forbids `--force`, so the marker is the **only** way past it. That is by design — Tim declined a metadata-only CI auto-merge specifically because he wanted agent judgement in the loop. An authorship-keyed SKIP was considered and rejected: it would have let Dependabot PRs merge with nothing having reviewed them at all.

This is also why the authorship chain stops where it does. Commit-signature verification kills the local `git commit --author=…` spoof, but it does not establish that only GitHub could have produced a commit — the Contents API sets `author` independently of `committer`, and genuine Dependabot commits already report `committer: GitHub`. The backstop against a commit appended to a Dependabot branch is the `reviewed` gate, not the authorship chain. Don't extend that chain with payload parsing, committer heuristics, or app-attribution checks; read the diff instead.
...
2. **Every commit** on the PR is Dependabot-authored **and carries a valid GitHub signature** (`verification.verified == true`). The signature half is what makes this real: a commit's author is settable metadata (`git commit --author="dependabot[bot] <…>"` reports `login: dependabot[bot]`), so a login-only check would not stop a forged commit pushed onto a `dependabot/*` branch. GitHub signs genuine Dependabot commits; a locally-forged one is `unsigned`. Either failure disqualifies the whole PR — it goes back to the `--human` path. **What this does not establish:** that only GitHub could have produced the commit. The Contents API sets `author` independently of `committer`, and `committer` can't discriminate here since genuine Dependabot commits are API-created too. By design, that gap is covered by the `reviewed` gate (you reading the diff), not by more authorship machinery — see the callout above step 3.
