...
CodeRabbit may finish an incremental review of a merge commit without posting a new native review. The gate accepts its exact-head `final_review_risk_coverage` summary from the CodeRabbit App only when its latest decisive native review was an earlier approval or an adjudicated finding review, both summary commit fields match the current head, and every finding thread is resolved. A `CHANGES_REQUESTED` review qualifies as adjudicated only when its stated actionable count matches its inline comments and each comment belongs to a resolved thread with an owner reply or a later change to its commented file; body-only and silently resolved findings remain blocked. This is CodeRabbit coverage, not a quota signal; do not request Codex or repeat the CodeRabbit request.
