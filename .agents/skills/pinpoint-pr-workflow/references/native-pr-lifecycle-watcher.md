...
2. On `outcome: "passed"`, promote a draft PR to trigger CodeRabbit; for a later head, run `bash scripts/workflow/request-coderabbit-review.sh <PR>`. Use `request-codex-review.sh <PR> <reply-ID>` only after a trusted usage-limit reply to the SHA-tagged request on the current head.
