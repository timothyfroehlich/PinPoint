...
| :---------- | :---------------------------------------- | :------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | `.codex/agents/pr-lifecycle-watcher.toml` | `gpt-5.3-codex-spark`, low | Read-only sandbox; `.codex/config.toml` registers the watcher MCP as non-fatal at session startup, with only `watch_pr_lifecycle` enabled for child inheritance. The owner lifecycle still fails closed if the named watcher is unavailable. |
