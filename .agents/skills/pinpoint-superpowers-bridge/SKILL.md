...
- **`requesting-code-review` does not satisfy the merge gate by itself.** Own the GitHub draft/CI/automatic-review loop through exact-head coverage with every finding thread adjudicated and resolved. Automatic review remains first for every head; after its bounded witness conclusively ends without exact-head evidence, one `@codex review` fallback is allowed for that unchanged head, never repeated. A new head restarts automatic-first. Local attestations still require Tim to run the named local review. Full rules: `pinpoint-pr-workflow` Phase 3.4.
