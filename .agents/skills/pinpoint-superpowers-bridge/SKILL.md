...
- Superpowers' reviewer-subagent is fine as an **optional local self-check**. The **authoritative** review gate is **CI Gate + the head-commit review requirement** in `pinpoint-pr-workflow`: Tim runs `/codex:review`, then you record the completed current-head result with `mark-review.sh`. Running a superpowers review alone does not satisfy that requirement.
- **`requesting-code-review` does not satisfy the merge gate by itself.** Stop iterating, then hand the branch to Tim for `/codex:review` and attest only that completed result. Full rules: `pinpoint-pr-workflow` Phase 3.4.
