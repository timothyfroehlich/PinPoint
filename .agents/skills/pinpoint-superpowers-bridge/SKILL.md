...
- **Never merge locally and never push/merge to `main`; the merge decision is Tim's.** Ship through a draft PR and follow `pinpoint-pr-workflow` through current-head CI, CodeRabbit-first review coverage, final labeling, and screenshots before handing Tim `! scripts/workflow/merge-pr.sh <PR> --human` — or running that script in Claude Code, where the hook requires his approval (PP-wi85). The raw channels — `gh pr merge`, `gh api PUT .../merge`, MCP merge — stay hard-blocked for agents.
