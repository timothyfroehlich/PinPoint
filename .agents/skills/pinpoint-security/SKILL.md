...
- **Mixed-content directives are HTTPS-only**: `block-all-mixed-content` and `upgrade-insecure-requests` are emitted on every Vercel production/preview deployment (keyed on `VERCEL_ENV`, so no request header can remove them) and on any other https request, but never over plain http. WebKit applies `upgrade-insecure-requests` to `http://localhost` too, rewriting every chunk to `https://localhost`, so local dev and E2E would never hydrate in Safari (PP-b0gz).
