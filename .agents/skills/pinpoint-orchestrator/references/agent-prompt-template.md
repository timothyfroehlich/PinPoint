...
Then self-review **by hand**: read your own diff (`git diff origin/main...HEAD`) against `REVIEW.md` — the canonical rubric — plus the bead's acceptance criteria and out-of-scope list, and fix what you find. Do not attest a review yourself: Claude Code must run `/codex:review --base main` after the final push.

A review covering the head commit is **required** to merge. The primary reviewer is Codex, launched by Claude Code with `/codex:review --base main`; getting reviewed is a handoff after all work is finished.
...
The marker pins a SHA, so any push after it invalidates it. Every changed head needs a fresh `/codex:review --base main` before re-attestation.
...
5. Once CI is green and you have stopped iterating, have Claude Code run `/codex:review --base main` — unless it qualifies for the trivial-change exception above
...
5. Nothing reviews automatically — after work stabilizes, Claude Code runs `/codex:review --base main`. At handoff the lead checks the marker's pinned SHA against head (SKILL.md → "Ensure every PR is reviewed")
