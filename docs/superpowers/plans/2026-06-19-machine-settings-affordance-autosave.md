...
- **Task 8** consolidate edit pipeline + reactive dirty signal (`editSet` chokepoint, `useSaveStatus` dirty dimension) — DONE `981557b0` + fix `cf817599` (Critical: rekey dirtyIds temp→real on insert, else the temp id leaked and permanently armed the nav guard). Both review stages passed.
- **Task 9** durability — flush-on-nav/unmount, TipTap `onBlur`, guard rework (prompt-only-on-failed, popstate, fire-and-forget flush) — DONE `8bcfefdf` + fix `3d722ce4` (Important: drive the leaving-flush from save-status, not the debounce-timer map, so a failed-then-popstate/unmount edit still re-persists). Both review stages passed.
- **Task 10** mobile `RowEditSheet` auto-save + keyboard-aware scroll — DONE `2624884f` + fix `b6e9d30b` (Important: carry `codeLike`/`enterKeyHint` to the mobile inputs so iOS doesn't autocapitalize setting IDs). Both review stages passed. Keyboard occlusion needs device verification on the preview (PP-jn45); `interactive-widget=resizes-content` viewport flagged for a follow-up (not changed — app-wide).
- **Task 11** machine-level fields — bug #6 (preset Replace now updates the editor via a `key`-bump remount) + draft nav-protection. **Scope corrected by Tim:** the two machine-level fields KEEP explicit Save/Cancel (the auto-save conversion was an oversight); instead their unsaved drafts now warn before leaving (prompt, via `dirtyDraftIds` → `hasUnsavedDraft` in the guard). DONE `9e8c9c9d`. Both review stages passed (1 pre-existing minor → PP-od8m).
- **Task 12** final integration pass — DONE. Bug-map #1/#3/#5/#7 confirmed dissolved in code. Merged `origin/main` (`5015bc25`), lockfile valid, `pnpm run check` green (165 files / 1394 tests). Preflight integration green (154); the only smoke failures are pre-existing collection-pages `/c/owner/[member]` seed-pagination (PP-ph46, unrelated to this feature). **Remaining (Tim-gated):** manual/visual verification on dev, mobile verification on the preview, push + PR. Follow-up beads filed: PP-ph46 (E2E pagination/user-cruft), PP-od8m (InlineEditableField phantom-dirty).
