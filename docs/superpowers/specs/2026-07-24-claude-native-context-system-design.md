...
Because the window is unpredictable, confirming the `Agent` matcher still belongs in a **fresh session started after the hooks are committed** — a pre-merge gate on PR 1 (§11).
