...
- **Database setup:** Playwright `global-setup` runs pre-flight checks (Supabase health, Postgres connectivity), applies pending migrations, then fast-resets the database (truncate + seed). If fast-reset fails (e.g., fresh checkout), it falls back to a full `supabase db reset` + migrate + seed. Set `SKIP_SUPABASE_RESET=true` to bypass entirely for iterative UI-only runs.
