...
- **Application endpoints and page loaders form the sole authorization perimeter**: Because the database does not enforce row security, all request entry points—Server Components and page data loaders (`src/app/**`), Server Actions (`"use server"`), Route Handlers (`src/app/api/**`), and MCP endpoints—must enforce authorization at the application layer before disclosing restricted data or persisting mutations.
