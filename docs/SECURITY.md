...
- **SECURITY DEFINER RPCs (Database-level boundary)**: Postgres functions returning secrets (e.g., `get_discord_config()`, `get_pinballmap_user_credentials()`) are exposed directly to HTTP callers over PostgREST (`POST /rest/v1/rpc/<name>`). Because Supabase re-grants `EXECUTE` on `public.*` functions to `authenticated` at connection time, `REVOKE`/`GRANT` alone is insufficient defense in depth. These functions enforce an explicit database-level authorization boundary via an in-body `IF COALESCE(auth.role(), '') <> 'service_role' THEN RAISE EXCEPTION ...` check (see migrations 0029 and 0062) to prevent authenticated members from retrieving Vault secrets.
