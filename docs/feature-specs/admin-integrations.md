...
- _Retired 2026-08-27._ The section's detailed sub-requirements moved to
  ...
  their exact successors. The former §4.1 (the section shows and edits the
  location id and read-only sync health) → `pinballmap.md` §10.1 — the §4.1 slot
  above is now the surviving section-level pointer. §4.2 → §10.2 · §4.3 → §10.3 ·
  §4.4 → §10.4 · §4.5 → §10 (introduction). The former §4.6 (per-operator write
  credentials are not configured here) is retained inline in §4.1 above.
