...
- **4.5** A quiet period with no add/remove events is a normal no-op, not a
  failure. Only a failed, malformed, or implausibly large/incomplete response is
  treated as a bad read and discarded without announcing.
