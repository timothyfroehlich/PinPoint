...
- **3.6** Full support also lets an authorized person explicitly confirm the tracked location's lineup (3.7) and bring an eligible entry's Insider Connected setting in line with PinPoint's Insider Connected intent (3.8). Neither runs automatically or changes PinPoint's listing intent. Posting condition comments from PinPoint is deferred until requested and is not part of full support.
...
- **3.8** A cabinet whose title Pinball Map's catalog marks Insider Connected eligible carries an **Insider Connected intent** (On, Off, or none) alongside its listing intent, whatever the listing intent is; an ineligible title has none, and the control shows nothing for it. Eligibility comes only from Pinball Map's catalog flag, never inferred from era or brand. Setting it follows 8.1, writes only to PinPoint, needs no confirmation, and is instantly reversible. With no intent recorded, the control shows Pinball Map's value (**On**, **Off**, or **Not set** when Pinball Map has none) and never flags it. Same-title cabinets share one entry, so the entry's target is On when any cabinet sharing it wants On, otherwise Off when any wants Off. While the entry is present and its target differs from Pinball Map's value, the control is Out of sync (4.2) and the push (4.3) carries the change. PinPoint sends the target setting itself, never a flip, so a stale page or a repeated click cannot invert it. An unclear outcome is never retried: PinPoint re-reads and shows the actual setting.

## 4. The listing control

- **4.1** The control is two rows under one header, and **renders at the same fixed height in every state** — states swap content, never geometry. On a narrow screen a status row's push action may wrap below its status sentence, and the Insider Connected switch sits below the intent toggle. An uncataloged machine has no control at all (§4.2).
  - **Intent row**: the tri-state toggle (On the lineup / Off the lineup / Don't sync), followed for an eligible title by the Insider Connected switch (3.8). Changing either writes only to PinPoint, needs no confirmation, and is instantly reversible.
  - **Insider Connected switch**: labelled On, Off, or Not set, with a warning icon naming Pinball Map's value when the two differ.
...
- **4.9** A signed-in member without the machine-linking capability sees the header and both rows, never the status row's push actions; the intent toggle and the Insider Connected switch render read-only. The header Refresh stays available to them (8.3).
