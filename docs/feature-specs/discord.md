...
on), the Pinball Map region-alerts spec (shares the bot token — §1.2).
...
- **1.1** One Discord bot, PinPoint-wide, delivers notifications as direct
  messages to members.
- **1.2** The bot token is configured here and also used by the Pinball Map
  region-alert channel posts (separate spec). Clearing the token turns off both
  (§2.6).
...
- **4.2** A member who cannot receive DMs is shown the invite link, so they can
  join the server and receive future notifications.
...
| §4.2 invite link on DM failure  | Invite link is stored but never surfaced; no consumer in the send path            | Surface it on a failed DM (`sendDm` lumps 403/404 as `blocked`, so detection is best-effort) |
