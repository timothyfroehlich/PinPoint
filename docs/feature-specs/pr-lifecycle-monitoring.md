...
| 8.2 | Codex is invoked when CodeRabbit quota is exhausted or when explicitly requested by an operator or agent | `request-codex-review.sh`, the agent's Codex path, requires CodeRabbit's `Review rate limited` reply to the owner's SHA-tagged `request-coderabbit-review.sh` request for the current head; without verified exhaustion an agent cannot request Codex. Tim can still comment `@codex review` directly. | Amend 8.2 and the review hierarchy glossary entry to quota exhaustion only (requirement diff needs Tim's approval) |
