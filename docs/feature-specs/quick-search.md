...
- **2.2** At desktop widths, the top bar presents a search control.
...
- **5.4** Closing quick search returns focus to the control that opened it.
- **5.5** Quick search presents distinct initial, loading, empty, and failure states.
- **5.6** A failure leaves the dialog and current query available for another attempt.
- **5.7** A response for an older query never replaces results for a newer query.
- **5.8** Desktop and mobile entry points open the same search experience and produce the same results.
...
- **6.3** Keyboard focus remains contained within the open search dialog.
...
| Requirements | Divergence                           | Resolution |
| :----------- | :----------------------------------- | :--------- |
| §2–§7        | Quick search is not yet implemented. | PP-b6g1    |
