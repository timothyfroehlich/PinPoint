...
- **Enforced by:** `permissions.deny` / `permissions.ask` in `.claude/settings.json`. (The `verify-guard-stack.cjs` SessionStart check covers only missing hook scripts and the merge deny rules.)
...
- **Severity:** High
- **Why:** PinballMap is a third-party community service we both read from and write to publicly. Their `llms.txt` sets explicit conduct (use bulk endpoints, don't poll in loops, store-and-reuse tokens, real-but-generous rate limits) and their `robots.txt` blocks AI crawlers from the website — the documented JSON API is the only sanctioned channel. Their data is CC BY-SA 4.0, so displaying it requires attribution + a link back — and since 2026-08-06 their `llms.txt` states that a surface showing data for a **specific location** must link to that location's listing (`https://pinballmap.com/map?by_location_id=<id>`), not the homepage. Ignoring this risks rate-limit bans and violates their terms.
- **Mandatory API token (from July 30 2026):** PBM is closing its previously-open API behind a required `api_token` (blog 2026-07-16). Once their `REQUIRE_API_TOKEN` gate flips on, **ALL v1 endpoints — reads included — require the token**. This is a **two-layer** auth model: the `api_token` (sent as the `X-Api-Token` header, tied to an approved account, global limit **120/min**) is the blanket access gate for reads _and_ writes; writes **also** still carry the writing member's identity (`user_email` + `user_token`) as query params. The `api_token` is a **platform capability, not tenant data** — PBM issues it to an approved account against a use-plan, i.e. to PinPoint-the-application — so it is a deploy-time constant read from the `PINBALLMAP_API_TOKEN` env var (`~/lib/pinballmap/api-token.ts`) and injected onto every request by `createLiveClient`. It is deliberately **not** in the `next.config.ts` build registry: without it PBM sync degrades and nothing else breaks (CORE-SEC-009; docs/ENV_VARS.md §4.2). Each member's write token stays in Supabase Vault (`pinballmap_user_credentials.token_vault_id`) because it is per-user identity arriving at runtime through the account-link flow (PP-o355.6). (PP-uusr; moved out of Vault by PP-o355.23 / migration 0059.)
- **Do:** Route all PBM access through the `PinballMapClient` seam (`~/lib/pinballmap`). Use the documented JSON API per the vendored `docs/external/pinballmap-llms.txt`. Sync cadence: the automated cron does one location call per hour; human-initiated refreshes draw from a global token bucket at the `syncLocationSnapshot` seam — `PBM_REFRESH_BURST` (3) back-to-back, then one per `PBM_REFRESH_REFILL_MS` (3 min), so the SUSTAINED rate is **≤20/hour** exactly. The token is spent on the ATTEMPT, so a failed fetch can't fail-open the guard (PP-hbi0). The bucket is global rather than per-user, because the cap is on PinPoint's traffic to their service; changing the refill changes that commitment, while changing the burst only changes how bunched the same traffic is (PP-o355.21). Store and reuse the `api_token` (env var) + operator tokens (Supabase Vault) and send `X-Api-Token` on every request; send a descriptive User-Agent with a contact URL; back off on 429; render attribution + a link back wherever PBM data appears, built by `pinballmapLocationUrl()` (`~/lib/pinballmap/public-url`) so it targets our location's listing rather than the homepage — never hand-write a pinballmap.com URL. Re-read the vendored `docs/external/pinballmap-*` files (refreshed on drift by the weekly chores routine — there is no drift GHA) before changing integration code.
