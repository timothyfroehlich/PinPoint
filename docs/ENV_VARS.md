...
| `PINBALLMAP_API_TOKEN`                           | 🔴    | ⭕                  | `src/lib/pinballmap/api-token.ts`         | read from env at runtime; absent → PBM `X-Api-Token` omitted, live reads/writes fail once PBM's REQUIRE_API_TOKEN gate flips (July 30 2026). Production-only for app runtime — never persist locally or set in CI. The manual fixture-refresh exception below may inject it ephemerally. Never `NEXT_PUBLIC_`, never reused. |
...
| `ISCORED_USER`                                   | 🟢    | ⚪                  | `src/lib/iscored/config.ts`               | iScored integration disabled; leaderboard cards and iScored tabs render quiet empty states. Public gameroom username (e.g. `Apcscore`), not a secret, but never `NEXT_PUBLIC_`.                                                                                                                                              |
...
> ```
>
> The script fails before making a request when the variable is absent. CI,
> previews, and normal local app runtime keep it unset and continue using the
> mock client (CORE-TEST-006).
>
...
| `MAILPIT_HOST`                                                                                       | 🟢            | Dev/CI   | `src/lib/email/transport.ts`                                                                   | Mailpit host; default `127.0.0.1` (`localhost` in E2E). Set by `worktree_setup.py` for a remote Supabase backend                           |
| `PINPOINT_SUPABASE_BACKEND`                                                                          | 🟢            | Dev      | `scripts/worktree_setup.py`, `scripts/supabase-stack.sh`                                       | `local` or `remote`; stored in `.env.local`. In the shell it is only the default for new worktrees. See `docs/runbooks/remote-supabase.md` |
| `PINPOINT_SET_SUPABASE_BACKEND`                                                                      | 🟢            | Internal | `scripts/worktree_setup.py`                                                                    | One-shot backend switch, set by `pnpm supabase:use`                                                                                        |
| `PINPOINT_REMOTE_DOCKER_HOST` / `PINPOINT_REMOTE_SUPABASE_HOST` / `PINPOINT_REMOTE_SUPABASE_NETWORK` | 🟢            | Dev      | `scripts/supabase-stack.sh`, `scripts/worktree_cleanup.py`, `scripts/worktree_orphan_sweep.py` | Remote backend: Docker endpoint, service hostname, network (default `pinpoint-dev-tailnet`). Shell only, never `.env.local`                |
| `PINPOINT_DEV_DB_HOSTS`                                                                              | 🟢            | Dev      | `scripts/assert-local-db.mjs`                                                                  | Comma-separated dev-stack hosts the local-only DB guards also accept; managed cloud hosts are refused regardless                           |
| `PINPOINT_RESERVED_SLOTS`                                                                            | 🟢            | Dev      | `scripts/worktree_setup.py`                                                                    | Comma-separated port slots new worktrees skip (stacks the remote host runs outside this manifest)                                          |
