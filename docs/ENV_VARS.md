...
| `DISCORD_PBM_ALERT_CHANNEL_ID`                            | 🟢    | ⚪                  | `src/lib/pinballmap/region-alerts.ts`     | region new-machine alert is off: the daily cron makes **no** PBM call and records nothing, so nothing is queued and nothing floods when it is later set. Not a secret (a channel snowflake), but never `NEXT_PUBLIC_`.         |
...
> is the Discord channel the daily "new machines in Austin" alert posts into. It
...
> in the meantime would land as one flood the day a channel was finally set.
