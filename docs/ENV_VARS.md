...
| `PINPOINT_SUPABASE_BACKEND`                                                     | 🟢            | Dev/test | `scripts/ensure-supabase.sh`, `scripts/dev-status.sh`, `e2e/global-setup.ts` | `remote` from Tim's Mac dotfiles selects Bazzite; `local` is explicit opt-in. Unset and CI use local. Never set in Vercel. |
