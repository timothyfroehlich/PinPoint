...
| `PINPOINT_REMOTE_DOCKER_HOST` / `PINPOINT_REMOTE_SUPABASE_HOST` / `PINPOINT_REMOTE_SUPABASE_NETWORK` | 🟢            | Dev      | `scripts/supabase-stack.sh`, `scripts/worktree_cleanup.py` | Remote backend: Docker endpoint, service hostname, network (default `pinpoint-dev-tailnet`). Shell only, never `.env.local`                |
