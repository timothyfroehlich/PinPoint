...
class SyncManager:
    ...
    def pre_flight_checks(self) -> bool:
        ...
        # 1. Check for running Supabase instances
        if not self._check_supabase_stopped():
            return False
        
        # 2. Check for legacy Docker volumes (warning only)
    ...
    def _check_supabase_stopped(self) -> bool:
        """Check that no Supabase instances are running"""
        print_status("info", "Checking for running Supabase instances...")
        
        if self.dry_run:
            print("[DRY-RUN] Would check for running Supabase instances")
            return True
        ...
        try:
            ...
            if running:
                ...
                print()
                print("The following Supabase containers are running:")
                print(running)
                print()
                print("These must be stopped before syncing to avoid port conflicts.")
                print()
                print("To fix:")
                print("  supabase stop --all")
                print()
