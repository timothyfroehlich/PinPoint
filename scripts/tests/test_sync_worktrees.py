...
class TestPortConfig:
    ...
    def test_all_worktrees_defined(self):
        ...
        expected = {"PinPoint", "PinPoint-Secondary", "PinPoint-review", "PinPoint-AntiGravity"}
    ...
    def test_port_uniqueness(self):
        ...
        assert len(nextjs_ports) == len(set(nextjs_ports)), "Next.js ports must be unique"
    ...
    def test_project_id_format(self):
        ...
        for name, config in PORT_MAPPINGS.items():
            ...
            assert config.project_id == expected_id or config.project_id == name.lower().replace("-", "-")
