...
async function checkDatabaseConnection() {
  ...
  try {
    ...
    console.log(
      `${colors.yellow}⚠️  Database connection issue (will retry)${colors.reset}`,
    );
  }
}
...
async function main() {
  console.log(
    `${colors.blue}🔄 Preparing for server restart...${colors.reset}`,
  );

  await Promise.all([clearNextCache(), checkDatabaseConnection()]);
