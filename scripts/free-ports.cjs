...
async function freePort(port) {
  ...
  try {
    ...
    if (pids.length === 0) {
      console.log(
        `${colors.green}✅ Port ${port} is already free${colors.reset}`,
      );
      return true;
    }

    console.log(
      `${colors.yellow}⚡ Found ${pids.length} process(es) using port ${port}${colors.reset}`,
    );
    ...
    for (const pid of pids) {
      try {
        ...
        console.log(
          `${colors.yellow}  📋 Process ${pid}: ${processInfo.trim().substring(0, 50)}...${colors.reset}`,
        );
    ...
    if (remainingPids.length === 0) {
      ...
      console.log(
        `${colors.red}❌ Port ${port} still has ${remainingPids.length} process(es)${colors.reset}`,
      );
      return false;
    }
  } catch (/** @type {any} */ error) {
    console.log(
      `${colors.red}❌ Error checking port ${port}: ${error.message}${colors.reset}`,
    );
    return false;
  }
}
...
async function main() {
  ...
  console.log(
    `${colors.bold}🔓 Freeing Ports: ${ports.join(", ")}${colors.reset}\n`,
  );
  ...
  for (const port of ports) {
    if (isNaN(port)) {
      console.log(
        `${colors.red}❌ Invalid port: ${args[ports.indexOf(port)]}${colors.reset}`,
      );
      allFreed = false;
      continue;
    }
  ...
  if (allFreed) {
    console.log(
      `${colors.green}✅ All requested ports are now free!${colors.reset}`,
    );
    console.log(
      `${colors.blue}🚀 You can now start your development servers${colors.reset}\n`,
    );
  } else {
    console.log(`${colors.red}❌ Some ports could not be freed${colors.reset}`);
    console.log(
      `${colors.yellow}💡 You may need to restart your system or check for system services${colors.reset}\n`,
    );
    process.exit(1);
  }
}
