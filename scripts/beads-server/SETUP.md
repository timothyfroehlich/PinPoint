...
- On a **pull conflict** the script checks `dolt_conflicts` on the live server.
  ...
  bridge runs `CALL DOLT_MERGE('--abort')` and verifies `dolt_conflicts` is empty
  before stopping. A human resolves the DoltHub divergence, then runs
