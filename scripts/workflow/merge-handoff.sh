...
elif [[ "$rv_state" == "approval" || "$rv_state" == "clean_comment" || "$rv_state" == "marker" ]]; then
...
if [[ "$rv_state" != "approval" && "$rv_state" != "clean_comment" && "$rv_state" != "marker" ]]; then add_block "reviewed: ${rv_state} — await a clean automatic Codex result on the current head; use a manual trigger only when Tim explicitly requests it"; fi
