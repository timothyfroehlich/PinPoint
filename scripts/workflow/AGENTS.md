...
**The merge decision is Tim's (PP-wi85, reversed for the script per Tim 2026-08-19).** The orchestrator stops after running `merge-handoff.sh` and presenting its computed state. `merge-pr.sh --human` is a later action that requires a new, explicit instruction from Tim; the `block-direct-merge.cjs` PreToolUse hook also turns the invocation into an approval prompt. The raw channels (`gh pr merge`, `gh api PUT .../merge`, MCP `merge_pull_request`) stay hard-blocked because they skip the script's gate re-checks. Agents run every non-merge script in this directory freely, including `pr-screenshots.mjs` and `merge-handoff.sh`.
...
| `merge-pr.sh <PR> --human [-a]` | **Merge decision is Tim's.** Run only after Tim explicitly instructs the agent to merge that PR; the hook then prompts him to approve. Re-evaluates all 4 gates and squash-merges if all pass. `--automerge` never waits out review; the owning agent monitors automatic review outside this script.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Merge a PR                                     | —                                              | `merge-pr.sh --human` after Tim explicitly instructs the merge; the hook prompts again                |
| Composite gate evaluation                      | —                                              | `merge-handoff.sh` for the pre-decision snapshot; `merge-pr.sh` only after explicit merge instruction |
...
- **The merge decision is Tim's (PP-wi85, reversed for the script per Tim 2026-08-19)**: orchestration ends with `merge-handoff.sh`. An agent runs `scripts/workflow/merge-pr.sh <PR> --human` only after a new, explicit merge instruction from Tim; the `.claude/hooks/block-direct-merge.cjs` PreToolUse hook then requires Tim to approve again. The raw channels `gh pr merge`, `gh api PUT .../merge`, and MCP `merge_pull_request` stay hard-blocked because they skip the script's gate re-checks. `merge-pr.sh` also refuses to execute a merge without `--human` at the script level, as defense-in-depth for harnesses that do not wire the Claude Code hook.
