...
| `_pr-gates.sh`                  | Shared bash helper sourced by `merge-pr.sh`, `merge-handoff.sh`, and `request-codex-review.sh`, and shelled out to by `pr-watch.py` and `pr-dashboard.py`. `_review_summary <PR>` is the one implementation of review evidence: two checkers (CodeRabbit approval, Codex evidence) and a four-word label.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
The agent reads these tokens from script stdout to decide next steps. The `reviewed` remedy says to comment `@coderabbitai review` once after current-head CI succeeds and the PR is ready (or run `request-codex-review.sh` once when CodeRabbit is rate-limited), then wait for evidence. Continuation lines are indented and carry no status token, so token parsing is unaffected.
