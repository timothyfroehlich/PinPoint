...
**CodeRabbit is the default GitHub reviewer.** After current-head CI succeeds, promoting the draft PR requests CodeRabbit automatically; later heads need `bash scripts/workflow/request-coderabbit-review.sh <PR>`. If draft promotion is rate-limited, make one SHA-tagged manual request on this head. Only a trusted `Review rate limited` reply to the SHA-tagged CodeRabbit request permits `request-codex-review.sh <PR> <reply-ID>`. The helper verifies the reply's bot and app identity, PR, SHA-tagged request, and response update timing before posting a SHA-bound `@codex review`. After `Review finished`, a missing native approval and missing trusted exact-head incremental summary coverage call for CodeRabbit follow-up. The gate accepts a CodeRabbit `APPROVED` review pinned to head, or its trusted exact-head incremental summary after a prior native approval. Codex evidence may be a native review, exact-head clean connector comment, or trusted reaction witness; finding threads remain governed by the separate thread gate. The existing SHA-pinned `mark-review.sh` route remains valid after Tim explicitly runs a local review. A new head requires replacement CI and review.
...
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `coderabbit` | Native `APPROVED` pinned to head; or trusted exact-head incremental summary after a prior native approval                                                                          | `changes_requested` (its `CHANGES_REQUESTED` on head), `stale` (off-head), `none` |
