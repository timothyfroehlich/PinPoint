...
| `pr-watch.py <PR>`                            | Stream CI run events. One timestamped line per event. Use with the Claude Code Monitor tool. The first host process for a repository+PR holds the XDG-state lock and polls GitHub; concurrent invocations follow its atomic local state with zero GitHub reads. A later invocation never trusts an unlocked terminal cache. Writes failure artifacts to `tmp/gh-monitor/`. Unresolved threads print a reminder but do **not** stop the watch. `--check-ready` remains a direct readiness snapshot rather than a shared monitor. |
