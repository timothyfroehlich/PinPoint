...
**Copilot review was retired on 2026-08-02 (PP-4ric).** The primary review that satisfies the gate is Codex, started by Claude Code with `/codex:review --base main`; Claude is permitted to trigger it itself. `mark-review.sh` records only a completed result for the exact head SHA. The exception is a genuinely trivial change (a typo, a comment, a one-line mechanical fix), where the marker summary should say why it was trivial.
...
**`stale_marker` is the one worth reading carefully.** It is the successor to the old `pushed_after` and the same trap: the PR visibly HAS a review, so the reflex is to read the gate as flaky rather than as "the commit about to merge was never looked at". Nothing re-attests automatically, and that is deliberate — a 3-commit fixup should not inherit the review of the commit before it. Re-run `/codex:review --base main` after every changed head.
...
The agent reads these tokens from script stdout to decide next steps. Scripts emit prescriptive advice only where the remedy is otherwise undiscoverable. That is the `reviewed` and `threads` gates: every FAIL is followed by indented continuation lines naming the action that clears it, PR number already substituted. The `reviewed` remedy names **both** steps — Claude runs `/codex:review --base main`, then the agent attests — because printing `mark-review.sh` alone reads as "attest and move on", which is the false attestation the gate exists to prevent. Continuation lines are indented and carry no status token, so token parsing is unaffected.
