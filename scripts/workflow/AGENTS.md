...
| `merge-pr.sh <PR> --human [-a]` | **Merge decision is Tim's — an agent may run it, the hook prompts him to approve.** Re-evaluates all 4 gates and squash-merges if all pass. `--automerge` never waits out review: comment `@codex review` and get a fresh Codex approval after every push.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `_pr-gates.sh`                  | Shared bash helper sourced by `merge-pr.sh` and `merge-handoff.sh`. Its native GitHub review lookup is the one implementation used by both gate and handoff.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `reviewed`    | Hard backstop — Codex must approve the current head. PASS on `approval`; FAIL on `stale_approval` / `not_approved` / `unreviewed` | `force`     |
| `no_conflict` | PR is MERGEABLE (never bypassable — GitHub rejects conflicting merges)                                                            | `none`      |

### Review state (`reviewed`)

**Codex GitHub review is the only review that satisfies this gate.** Tim triggers it by commenting `@codex review` on the PR; it uses the GitHub integration and does not require an OpenAI API key. The gate reads GitHub's native review record from exact account `chatgpt-codex-connector[bot]`, requires `APPROVED`, and requires its `commit_id` to equal the current PR head SHA. Every push needs a fresh comment and approval. Manual local `/codex:review`, built-in `/code-review`, and `mark-review.sh` attestations do not satisfy the gate.

`_compute_review_state` in `_pr-gates.sh` reports four states:
...
| `unreviewed`     | Codex has not reviewed this PR                    | FAIL       |

The latest trusted Codex review is authoritative. A later `CHANGES_REQUESTED` or `COMMENTED` review overrides an earlier approval, and an approval of an older commit is stale; neither can cover the commit about to merge.

Nothing here WAITs. A review cannot become valid just by waiting: an unreviewed or stale head needs `@codex review`. `merge-pr.sh --automerge` must stop rather than poll for somebody to notice it.
...
The agent reads these tokens from script stdout to decide next steps. The `reviewed` remedy tells Tim to comment `@codex review` on the already-substituted PR number and repeat it after any push. Continuation lines are indented and carry no status token, so token parsing is unaffected.
