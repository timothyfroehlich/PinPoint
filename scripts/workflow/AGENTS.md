...
Tim decides whether PinPoint PRs merge (PP-wi85). When he explicitly requests a merge in the active task and the PR target is unambiguous, the owning agent in any harness runs `merge-pr.sh <PR> --human`; the script rechecks all four gates. Claude Code and Codex may add a hook approval prompt; the direct request authorizes the agent in every harness. Without that request, use `merge-handoff.sh`. Raw PinPoint merge channels remain prohibited for agents because they skip the gates. This boundary covers implicit current-repository and explicit `timothyfroehlich/PinPoint` targets; a statically explicit non-PinPoint target follows its repository policy. Environment-only selectors fail closed. The normal close follows `pinpoint-pr-workflow`: draft PR, current-head CI, CodeRabbit review (Codex only after a verified usage-limit reply), exact-head coverage, resolved threads, final label, then handoff.
...
| `request-coderabbit-review.sh <PR>`           | Request one SHA-tagged CodeRabbit review for the current head after CI passes. Refuses duplicate requests, covered or moving heads, in-progress reviews, draft/closed PRs, and non-owner auth.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `request-codex-review.sh <PR> <reply-ID>`     | Request one Codex review after a trusted current-head CodeRabbit usage-limit reply. Refuses a missing or stale rate-limit reply, non-owner auth, draft/closed PRs, non-green current-head CI, heads already reviewed/requested, and a head that moves during validation. Posts the SHA-bound `@codex review` comment consumed by the trusted reaction witness.                                                                                                                                                                                                                                                                                                                                                                                                               |
...
**CodeRabbit is the default GitHub reviewer.** After current-head CI succeeds, promoting the draft PR requests CodeRabbit automatically; later heads need `bash scripts/workflow/request-coderabbit-review.sh <PR>`. If draft promotion is rate-limited, make one SHA-tagged manual request on this head. Only a trusted `Review rate limited` reply to the SHA-tagged CodeRabbit request permits `request-codex-review.sh <PR> <reply-ID>`. The helper verifies the reply's bot and app identity, PR, SHA-tagged request, and response update timing before posting a SHA-bound `@codex review`. After `Review finished`, a missing native approval and missing trusted exact-head incremental summary coverage call for CodeRabbit follow-up. The gate accepts a CodeRabbit `APPROVED` review pinned to head, an exact-head finding review once its counted inline comments each have a resolved thread and owner reply or later file change, or its trusted exact-head incremental summary after a prior approved or adjudicated native review. Codex evidence may be a native review, exact-head clean connector comment, or trusted reaction witness; finding threads remain governed by the separate thread gate. The existing SHA-pinned `mark-review.sh` route remains valid after Tim explicitly runs a local review. A new head requires replacement CI and review.
...
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| `coderabbit` | Native `APPROVED` pinned to head; exact-head finding review with counted inline comments in resolved threads and owner replies or later file changes; or trusted incremental summary after a prior qualifying native review | `changes_requested` (its `CHANGES_REQUESTED` on head), `stale` (off-head), `none` |
...
The agent reads these tokens from script stdout to decide next steps. The `reviewed` remedy directs a CodeRabbit request after current-head CI; a verified usage-limit reply opens the Codex helper. Local attestation remains reserved for a local review Tim explicitly runs. Continuation lines are indented and carry no status token, so token parsing is unaffected.
...
| Request a CodeRabbit re-review                 | —                                              | `request-coderabbit-review.sh <PR>`                                                |
| Request a Codex review after usage exhaustion  | —                                              | `request-codex-review.sh <PR> <reply-ID>`                                          |
