...
| `review-preflight.sh <PR>`                          | Checks that a review launched here would actually cover this PR — right branch, local HEAD == the pushed head, clean tree, non-empty `main...HEAD` — then prints both review commands for Tim (`/codex:review` and `/code-review`) and the matching `mark-review.sh` pair for each. Prints **no** command when a check fails. Both reviewers read local git state and never read the PR, so a run from the wrong worktree finds nothing and is indistinguishable from a clean review; this is the step that catches that. The Codex base is always `main` and is not a parameter — it has to match a pair `mark-review.sh` accepts, and only `base-main` is one. Exit 0 ready, 1 not ready, 2 usage.                                                                                                                              |
