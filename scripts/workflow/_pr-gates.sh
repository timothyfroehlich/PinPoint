...
_review_record() {
  ...
  # A current Codex approval is already sufficient, so do not spend another paginated
  # API request fetching manual markers. Every other Codex state needs the independent
  # marker lookup: a current marker remains valid after a non-approval Codex review.
  marker=$(_marker_record "$@")
  marker_state=$(cut -f1 <<< "$marker")
  if [[ "$marker_state" == "marker" ]]; then
    printf '%s\n' "$marker"
  elif [[ "$codex_state" != "unreviewed" ]]; then
    if [[ "$marker_state" == "unreviewed" ]]; then
      printf '%s\n' "$codex"
    else
      # Neither record covers head. Report the newest actual review artifact so the
      # handoff can compute the diff since the most recent reviewed commit.
