...
async function setupRLS(): Promise<void> {
  ...
  try {
    ...
    console.log('   - Begin tRPC router conversion to remove manual org filtering');
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('permission denied')) {
        ...
        console.error('   - Ensure your database user has sufficient privileges');
        console.error('   - Check if you are connected to the correct database');
        console.error('   - Verify Supabase project permissions if using hosted database');
      }
      
      if (error.message.includes('relation') && error.message.includes('does not exist')) {
        console.error('');
        console.error('💡 Troubleshooting:');
        console.error('   - Run database migration first: npm run db:push:local');
        console.error('   - Ensure all schema files are up to date');
        console.error('   - Check if database was reset without schema migration');
      }
      ...
      if (error.message.includes('authentication failed')) {
        ...
        console.error('   - Ensure local Supabase is running if using local development');
      }
    } else {
