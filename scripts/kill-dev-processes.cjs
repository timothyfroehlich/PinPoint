...
async function killProcessesOnPorts(ports) {
  console.log(
    `${colors.blue}🔍 Finding processes on ports: ${ports.join(", ")}...${colors.reset}`,
  );
      ...
      if (pids.length > 0) {
        ...
        for (const pid of pids) {
          try {
            ...
            console.log(
              `${colors.green}  ✅ Sent TERM signal to process ${pid}${colors.reset}`,
            );
...
async function killNodeProcesses() {
  console.log(
    `${colors.blue}🔍 Finding Node.js development processes...${colors.reset}`,
  );

  try {
    const { stdout } = await execPromise(
      `ps aux | grep -E "(next|nodemon|tsc|concurrently)" | grep -v grep || echo ""`,
    );

    if (stdout.trim()) {
      const lines = stdout.trim().split("\n");
      console.log(
        `${colors.yellow}⚡ Found ${lines.length} development process(es)${colors.reset}`,
      );
      ...
      for (const line of lines) {
        ...
        try {
          await execPromise(`kill -TERM ${pid}`);
          console.log(
            `${colors.green}  ✅ Killed: ${command.substring(0, 50)}...${colors.reset}`,
          );
        } catch (/** @type {any} */ error) {
          console.log(
            `${colors.red}  ❌ Failed to kill ${pid}: ${error.message}${colors.reset}`,
          );
        }
      }
    } else {
      console.log(
        `${colors.green}✅ No development processes found${colors.reset}`,
      );
    }
  } catch (/** @type {any} */ error) {
    console.log(
      `${colors.yellow}⚠️  Could not check for Node processes: ${error.message}${colors.reset}`,
    );
  }
}
...
async function main() {
  console.log(
    `${colors.bold}🛑 Killing All Development Processes${colors.reset}\n`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.blue}💡 You can now safely restart with: npm run dev:full${colors.reset}\n`,
    );
  } catch (/** @type {any} */ error) {
    console.error(
      `${colors.red}❌ Failed to kill processes: ${error.message}${colors.reset}`,
    );
    process.exit(1);
  }
}
