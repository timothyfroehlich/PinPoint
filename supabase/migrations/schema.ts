import { pgTable, text, timestamp, unique, index, pgPolicy, boolean, integer, json, varchar, uuid, real, foreignKey, jsonb, inet, primaryKey, pgView, pgEnum } from "drizzle-orm/pg-core"
...
export const commenterType = pgEnum("commenter_type", ['authenticated', 'anonymous'])
...
export const reporterType = pgEnum("reporter_type", ['authenticated', 'anonymous'])
...
export const collections = pgTable("collections", {
	...
	pgPolicy("collections_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
]);
...
export const notifications = pgTable("notifications", {
	...
	pgPolicy("notifications_user_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`(user_id = (auth.uid())::text)`, withCheck: sql`(user_id = (auth.uid())::text)`  }),
]);
...
export const collectionTypes = pgTable("collection_types", {
	...
	pgPolicy("collection_types_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
]);
...
export const pinballMapConfigs = pgTable("pinball_map_configs", {
	...
	createMissingModels: boolean("create_missing_models").default(true).notNull(),
	updateExistingData: boolean("update_existing_data").default(false).notNull(),
}, (table) => [
	index("pinball_map_configs_organization_id_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops")),
	unique("pinball_map_configs_organization_id_unique").on(table.organizationId),
	pgPolicy("pinball_map_configs_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
]);
...
export const users = pgTable("users", {
	...
	notificationFrequency: notificationFrequency("notification_frequency").default('IMMEDIATE').notNull(),
}, (table) => [
	unique("users_email_unique").on(table.email),
	pgPolicy("users_self_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((id = (auth.uid())::text) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((id = (auth.uid())::text) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
	pgPolicy("users_no_anon_access", { as: "permissive", for: "all", to: ["anon"] }),
]);
...
export const attachments = pgTable("attachments", {
	...
	pgPolicy("attachments_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM issues i
  WHERE ((i.id = attachments.issue_id) AND (i.organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_effective_issue_public(i.id)))))` }),
	pgPolicy("attachments_member_read", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("attachments_anon_insert_public", { as: "permissive", for: "insert", to: ["anon"] }),
	pgPolicy("attachments_member_insert_permission", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("attachments_delete_permission", { as: "permissive", for: "delete", to: ["authenticated"] }),
]);
...
export const upvotes = pgTable("upvotes", {
	...
	index("upvotes_anon_session_issue_idx").using("btree", table.issueId.asc().nullsLast().op("text_ops"), table.anonymousSessionId.asc().nullsLast().op("text_ops")),
...
export const anonymousRateLimits = pgTable("anonymous_rate_limits", {
	...
	index("rate_limits_cleanup_idx").using("btree", table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("rate_limits_session_org_action_idx").using("btree", table.sessionId.asc().nullsLast().op("text_ops"), table.organizationId.asc().nullsLast().op("text_ops"), table.actionType.asc().nullsLast().op("text_ops")),
	pgPolicy("anonymous_rate_limits_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
	pgPolicy("anonymous_rate_limits_system_insert", { as: "permissive", for: "insert", to: ["anon"] }),
]);
...
export const issues = pgTable("issues", {
	...
	reporterType: reporterType("reporter_type").default('authenticated').notNull(),
	reporterEmail: text("reporter_email"),
	submitterName: text("submitter_name"),
	anonymousSessionId: varchar("anonymous_session_id", { length: 255 }),
	anonymousContactMethod: varchar("anonymous_contact_method", { length: 255 }),
	severity: severity().default('medium').notNull(),
	...
	index("issues_anon_session_idx").using("btree", table.anonymousSessionId.asc().nullsLast().op("text_ops")),
	...
	index("issues_moderation_pending_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops"), table.moderationStatus.asc().nullsLast().op("enum_ops")),
	...
	index("issues_public_org_idx").using("btree", table.organizationId.asc().nullsLast().op("bool_ops"), table.isPublic.asc().nullsLast().op("bool_ops")),
	index("issues_reporter_type_idx").using("btree", table.reporterType.asc().nullsLast().op("enum_ops"), table.organizationId.asc().nullsLast().op("text_ops")),
	index("issues_severity_idx").using("btree", table.severity.asc().nullsLast().op("enum_ops")),
	index("issues_status_id_idx").using("btree", table.statusId.asc().nullsLast().op("text_ops")),
	pgPolicy("issues_anon_create", { as: "permissive", for: "insert", to: ["anon"], withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (reporter_type = 'anonymous'::reporter_type) AND (created_by_id IS NULL) AND (EXISTS ( SELECT 1
  ...
  WHERE ((m.id = issues.machine_id) AND (m.deleted_at IS NULL)))))`  }),
	pgPolicy("issues_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"] }),
	pgPolicy("issues_member_select_all", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("issues_member_insert", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("issues_update_owner_or_perms", { as: "permissive", for: "update", to: ["authenticated"] }),
	pgPolicy("issues_delete_owner_or_perms", { as: "permissive", for: "delete", to: ["authenticated"] }),
]);
...
export const priorities = pgTable("priorities", {
	...
	pgPolicy("priorities_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
]);
...
export const models = pgTable("models", {
	...
	pgPolicy("models_global_read", { as: "permissive", for: "select", to: ["anon", "authenticated"], using: sql`true` }),
]);
...
export const invitations = pgTable("invitations", {
	...
	index("invitations_org_email_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops"), table.email.asc().nullsLast().op("text_ops")),
	index("invitations_organization_id_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops")),
...
export const rolePermissions = pgTable("role_permissions", {
	...
	index("role_permissions_permission_id_idx").using("btree", table.permissionId.asc().nullsLast().op("text_ops")),
	index("role_permissions_role_id_idx").using("btree", table.roleId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.roleId],
			foreignColumns: [roles.id],
...
export const permissions = pgTable("permissions", {
	...
	pgPolicy("permissions_global_read", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
]);
...
export const machines = pgTable("machines", {
	...
	ownerNotificationsEnabled: boolean("owner_notifications_enabled").default(true).notNull(),
	notifyOnNewIssues: boolean("notify_on_new_issues").default(true).notNull(),
	notifyOnStatusChanges: boolean("notify_on_status_changes").default(true).notNull(),
	...
	index("machines_public_org_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops"), table.isPublic.asc().nullsLast().op("text_ops")),
	...
	pgPolicy("machines_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (deleted_at IS NULL) AND (EXISTS ( SELECT 1
   FROM organizations o
  WHERE ((o.id = machines.organization_id) AND ((machines.is_public IS TRUE) OR ((machines.is_public IS NULL) AND (EXISTS ( SELECT 1
	...
	pgPolicy("machines_member_select_all", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("machines_member_insert", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("machines_update_owner_or_perms", { as: "permissive", for: "update", to: ["authenticated"] }),
	pgPolicy("machines_delete_permission", { as: "permissive", for: "delete", to: ["authenticated"] }),
]);
...
export const activityLog = pgTable("activity_log", {
	...
	index("activity_log_org_time_idx").using("btree", table.organizationId.asc().nullsLast().op("timestamp_ops"), table.createdAt.asc().nullsLast().op("timestamp_ops")),
	index("activity_log_user_id_idx").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.organizationId],
			foreignColumns: [organizations.id],
...
export const systemSettings = pgTable("system_settings", {
	...
	index("system_settings_org_key_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops"), table.settingKey.asc().nullsLast().op("text_ops")),
	index("system_settings_organization_id_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.organizationId],
			foreignColumns: [organizations.id],
...
export const issueStatuses = pgTable("issue_statuses", {
	...
	pgPolicy("issue_statuses_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND fn_is_org_member((auth.uid())::text, current_setting('app.current_organization_id'::text, true)))`  }),
]);
...
export const comments = pgTable("comments", {
	...
	commenterType: commenterType("commenter_type").default('authenticated').notNull(),
	anonymousSessionId: varchar("anonymous_session_id", { length: 255 }),
	...
	index("comments_anon_session_idx").using("btree", table.anonymousSessionId.asc().nullsLast().op("text_ops")),
	index("comments_author_id_idx").using("btree", table.authorId.asc().nullsLast().op("text_ops")),
	index("comments_commenter_type_idx").using("btree", table.commenterType.asc().nullsLast().op("text_ops"), table.issueId.asc().nullsLast().op("text_ops")),
	index("comments_issue_id_idx").using("btree", table.issueId.asc().nullsLast().op("text_ops")),
	index("comments_moderation_pending_idx").using("btree", table.issueId.asc().nullsLast().op("text_ops"), table.moderationStatus.asc().nullsLast().op("enum_ops")),
	index("comments_organization_id_idx").using("btree", table.organizationId.asc().nullsLast().op("text_ops")),
	pgPolicy("comments_anon_create_public", { as: "permissive", for: "insert", to: ["anon"], withCheck: sql`((EXISTS ( SELECT 1
   FROM issues i
  ...
  WHERE ((o.id = ( SELECT i2.organization_id
	...
	pgPolicy("comments_member_read", { as: "permissive", for: "select", to: ["authenticated"] }),
	pgPolicy("comments_member_insert", { as: "permissive", for: "insert", to: ["authenticated"] }),
	pgPolicy("comments_author_update", { as: "permissive", for: "update", to: ["authenticated"] }),
	pgPolicy("comments_author_delete", { as: "permissive", for: "delete", to: ["authenticated"] }),
	pgPolicy("comments_moderate_update", { as: "permissive", for: "update", to: ["authenticated"] }),
	pgPolicy("comments_moderate_delete", { as: "permissive", for: "delete", to: ["authenticated"] }),
	pgPolicy("comments_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"] }),
	pgPolicy("comments_auth_create_public", { as: "permissive", for: "insert", to: ["authenticated"] }),
]);
...
export const publicOrganizationsMinimal = pgView("public_organizations_minimal", {	id: text(),
	name: text(),
	subdomain: text(),
	logoUrl: text("logo_url"),
}).with({"securityBarrier":true}).as(sql`SELECT id, name, subdomain, logo_url FROM fn_public_organizations_minimal() fn_public_organizations_minimal(id, name, subdomain, logo_url)`);