import { pgTable, text, timestamp, unique, pgPolicy, boolean, index, real, integer, foreignKey, jsonb, inet, json, varchar, uuid, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const users = pgTable("users", {
	...
	pgPolicy("users_self_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`(id = (auth.uid())::text)`, withCheck: sql`(id = (auth.uid())::text)`  }),
...
export const locations = pgTable("locations", {
	...
	pgPolicy("locations_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND ((is_public = true) OR (is_public IS NULL)))` }),
...
export const organizations = pgTable("organizations", {
	...
	pgPolicy("organizations_public_read", { as: "permissive", for: "select", to: ["anon"], using: sql`true` }),
	pgPolicy("organizations_auth_read", { as: "permissive", for: "select", to: ["authenticated"] }),
...
export const memberships = pgTable("memberships", {
	...
	pgPolicy("memberships_self_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`(user_id = (auth.uid())::text)`, withCheck: sql`(user_id = (auth.uid())::text)`  }),
...
export const machines = pgTable("machines", {
	...
	pgPolicy("machines_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND ((is_public = true) OR (is_public IS NULL)))` }),
	pgPolicy("machines_member_all_access", { as: "permissive", for: "all", to: ["authenticated"] }),
]);
...
export const roles = pgTable("roles", {
	...
	pgPolicy("roles_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
...
export const issues = pgTable("issues", {
	...
	pgPolicy("issues_anon_create", { as: "permissive", for: "insert", to: ["anon"], withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (reporter_type = 'anonymous'::reporter_type) AND (created_by_id IS NULL))`  }),
	pgPolicy("issues_public_read", { as: "permissive", for: "select", to: ["anon", "authenticated"] }),
	pgPolicy("issues_member_access", { as: "permissive", for: "all", to: ["authenticated"] }),
]);
...
export const priorities = pgTable("priorities", {
	...
	pgPolicy("priorities_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
...
export const issueStatuses = pgTable("issue_statuses", {
	...
	pgPolicy("issue_statuses_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
...
export const comments = pgTable("comments", {
  ...
  WHERE ((i.id = comments.issue_id) AND (i.organization_id = current_setting('app.current_organization_id'::text, true)) AND ((i.is_public = true) OR (i.is_public IS NULL))))) AND (commenter_type = 'anonymous'::commenter_type) AND (author_id IS NULL))`  }),
	pgPolicy("comments_member_access", { as: "permissive", for: "all", to: ["authenticated"] }),
]);
...
export const collections = pgTable("collections", {
	...
	pgPolicy("collections_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
...
export const collectionTypes = pgTable("collection_types", {
	...
	pgPolicy("collection_types_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
...
export const pinballMapConfigs = pgTable("pinball_map_configs", {
	...
	pgPolicy("pinball_map_configs_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
...
export const anonymousRateLimits = pgTable("anonymous_rate_limits", {
	...
	pgPolicy("anonymous_rate_limits_member_access", { as: "permissive", for: "all", to: ["authenticated"], using: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`, withCheck: sql`((organization_id = current_setting('app.current_organization_id'::text, true)) AND (EXISTS ( SELECT 1
   FROM memberships m
  WHERE ((m.user_id = (auth.uid())::text) AND (m.organization_id = current_setting('app.current_organization_id'::text, true))))))`  }),
