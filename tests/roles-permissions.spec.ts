...
class RolePermissionPage {
  ...
  async deleteRole(roleName: string, reassignTo?: string) {
    ...
    await this.page.click(
      `tr:has-text("${roleName}") button:has-text("Delete")`,
    );
  ...
  async assignUserToRole(userEmail: string, roleName: string) {
    ...
    await this.page.click(
      `tr:has-text("${userEmail}") button:has-text("Edit")`,
    );
  ...
  async verifyPermissionAccess(permission: string, shouldHaveAccess: boolean) {
    ...
    const permissionElements = {
      ...
      [TEST_PERMISSIONS.ORGANIZATION_MANAGE]:
        'nav a:has-text("Organization Settings")',
      [TEST_PERMISSIONS.ROLE_MANAGE]: 'nav a:has-text("Roles & Permissions")',
    };

    const selector =
      permissionElements[permission as keyof typeof permissionElements];
...
test.describe("Role Management UI", () => {
  ...
  test("admin can create new roles", async ({ page }) => {
    ...
    await expect(
      page.locator('tr:has-text("Custom Role") td:has-text("2")'),
    ).toBeVisible(); // 2 permissions
  });
  ...
  test("admin can edit existing roles", async ({ page }) => {
    ...
    // Assert
    await expect(
      page.locator('tr:has-text("Test Role") td:has-text("3")'),
    ).toBeVisible(); // 3 permissions
  });
  ...
  test("admin can delete roles with member reassignment", async ({ page }) => {
    ...
    await rolePage.createRole("Role to Delete", [
      TEST_PERMISSIONS.ISSUE_CREATE,
    ]);
    ...
    // Assert
    await expect(
      page.locator('tr:has-text("Role to Delete")'),
    ).not.toBeVisible();
  });
  ...
  test("cannot delete system roles", async ({ page }) => {
    ...
    await expect(
      page.locator('tr:has-text("Admin") button:has-text("Delete")'),
    ).not.toBeVisible();
    await expect(
      page.locator('tr:has-text("User") button:has-text("Delete")'),
    ).not.toBeVisible();
    await expect(
      page.locator('tr:has-text("Technician") button:has-text("Delete")'),
    ).not.toBeVisible();
  });
  ...
  test("displays role permissions correctly", async ({ page }) => {
    ...
    await expect(page.locator(".permission-list")).toContainText(
      "organization:manage",
    );
...
test.describe("Permission-Based UI Access Control", () => {
  ...
  test("admin has access to all features", async ({ page }) => {
    ...
    await rolePage.verifyPermissionAccess(
      TEST_PERMISSIONS.ORGANIZATION_MANAGE,
      true,
    );
...
test.describe("User Management and Role Assignment", () => {
  ...
  test("admin can assign roles to users", async ({ page }) => {
    ...
    await expect(
      page.locator(
        `tr:has-text("${TEST_USERS.user.email}") td:has-text("Technician")`,
      ),
    ).toBeVisible();
  });

  test("role assignment affects user permissions immediately", async ({
    page,
  }) => {
    // Arrange
    await rolePage.loginAs("admin");
  ...
  test("cannot assign non-existent roles", async ({ page }) => {
    ...
    // Act
    await page.click(
      `tr:has-text("${TEST_USERS.user.email}") button:has-text("Edit")`,
    );

    // Assert
    await expect(
      page.locator('select[name="role"] option:has-text("NonExistentRole")'),
    ).not.toBeVisible();
  });
...
test.describe("Organization Isolation", () => {
  ...
  test("roles are organization-specific", async ({ page }) => {
    ...
    // Act
    await rolePage.createRole("Org Specific Role", [
      TEST_PERMISSIONS.ISSUE_CREATE,
    ]);
    ...
    await expect(
      page.locator('tr:has-text("Org Specific Role")'),
    ).toBeVisible();
...
test.describe("Permission Validation and Security", () => {
  ...
  test("system roles cannot be modified", async ({ page }) => {
    ...
    await expect(
      page.locator('tr:has-text("Admin") button:has-text("Edit")'),
    ).not.toBeVisible();
    await expect(
      page.locator('tr:has-text("User") button:has-text("Edit")'),
    ).not.toBeVisible();
    await expect(
      page.locator('tr:has-text("Technician") button:has-text("Edit")'),
    ).not.toBeVisible();
  });
  ...
  test("permission dependencies are validated", async ({ page }) => {
    ...
    await expect(
      page.locator(`input[value="${TEST_PERMISSIONS.ISSUE_CREATE}"]`),
    ).toBeChecked();
  });
});
