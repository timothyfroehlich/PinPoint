...
test.describe("Dev Login Authentication Flow", () => {
  ...
  test("confirms dev login authentication works correctly", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Escher Lefkoff" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Harry Williams" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Tim Froehlich" }),
    ).toBeVisible();
  ...
  test("verifies dev login is only visible in development", async ({
    page,
  }) => {
...
test.describe("Authentication API Endpoints", () => {
  test("dev users endpoint returns test users", async ({ page }) => {
    ...
    const userEmails = data.users.map(
      (user: unknown) => (user as { email: string }).email,
    );
